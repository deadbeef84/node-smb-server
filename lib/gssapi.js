...
function GetSPNEGOTokenInitBytes() {
  return Buffer.from(spnegoTokenInitBytes);
}

function AcceptSecurityContext(context, inputToken) {
...
function negotiate(context, token) {
  ...
  for (const flag of flags) {
    console.log(
      negMsg.flags2 & ntlmssp[flag] ? '*' : ' ',
      flag,
      ntlmssp[flag].toString(2)
    );
  }
  ...
  return [
    ...
    Buffer.from(negTokenResp.toBER()),
  ];
}
