...
function negotiate(context, token) {
  ...
  logger.trace({
    ...negMsg,
    flags2: enumToStr(negMsg.flags2, _.pick(NTLMSSP, flags))
  }, 'ntlmssp negotiate')
...
function authenticate(context, token) {
  ...
  logger.trace({ authMsg, context }, 'ntlmssp authenticate')
  ...
  context.user = authMsg.user
  context.domain = authMsg.domain
