...
const reqlog = baseLogger.child({ module: 'request' })
  ...
  return request(options, cb)
    .on('response', function (res) {
      ...
      res.on('end', function () {
        ...
        if (transferred > 0) {
          let rate = Math.round(transferred / elapsed)
          let measure = 'b'
          if (rate >= 1024) {
            rate /= 1024 // kb
            measure = 'kb'
          ...
          rateText =
            '[' + transferred + 'b][' + Math.round(rate * 10) / 10 + measure + '/' + time + ']'
        }

        reqlog.info(
          '[%d] <- %d %s %s [%d to %d][%dms]%s',
          ts,
          res.statusCode,
          method,
          url,
...
function monitorTransferProgress(transfer, serverPath, fullPath, totalSize, progressCallback) {
  ...
  transfer.on('data', function (chunk) {
    ...
    const elapsed = currCheck - startTime
    if (elapsed > 0) {
...
export { monitorTransferProgress }
