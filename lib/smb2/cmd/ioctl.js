...
function handle(msg, commandId, body, related, connection, server, cb) {
  ...
  const input = body.slice(inputStart, inputStart + req.inputCount);
  const outputStart = 56 + req.outputOffset;
  const output = body.slice(outputStart, outputStart + req.outputCount);
  ...
  if (
    req.ctlCode == FSCTL_DFS_GET_REFERRALS ||
    req.ctlCode == FSCTL_DFS_GET_REFERRALS_EX
  ) {
  ...
  if (
    ...
    if (
      !req.fileId.equals(Buffer.from('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', 'hex'))
    ) {
    ...
    /*
    const openFile = session.GetOpenFileObject(req.fileId);
    if (openFile == null) {
