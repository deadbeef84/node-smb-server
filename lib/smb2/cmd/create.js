...
export default async function handle(msg, related, connection, server) {
  ...
  console.log('CREATE', msg.tree.share, name, req)
...
export function createResponse(x) {
  ...
  const body = put()
    ...
    .word64le(dateToWindowsFileTime(creationTime))
    .word64le(dateToWindowsFileTime(lastAccessTime))
    .word64le(dateToWindowsFileTime(lastWriteTime))
    .word64le(dateToWindowsFileTime(changeTime))
