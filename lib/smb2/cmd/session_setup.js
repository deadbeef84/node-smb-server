...
export default async function handle(msg, related, connection, server) {
  ...
  if (!msg.header.sessionId) {
    // create session
    const smbSession = new SMBSession(server)

    server.sessions[smbSession.uid] = smbSession
    connection.sessions[smbSession.uid] = smbSession

    msg.header.sessionId = smbSession.uid

  }

  const securityBuffer = msg.buf.slice(req.securityBufferOffset)
  logger.debug({...req, securityBuffer}, 'SESSION_SETUP')
  ...
  if (status === ntstatus.STATUS_SUCCESS) {
    ...
    const { user, domain, sessionKey } = connection.authContext
