...
function decode(buf) {
  ...
  var body = buf.slice(
    ...
    raw.nextCommand ? raw.nextCommand : buf.length
  );
...
function encode(msg, key, pad) {
  ...
  out
    ...
    .pad(16) // TODO: sign using crypto.createHmac('sha256', ...)
    // body
    .put(msg.body);

  let buf = out.buffer()

  console.log(msg.header.commandId, key?.toString('hex'), pad)
  if (key) {
    console.log('message1', buf.toString('hex'))
    ...
    // SMB3
    // const signature = aesCmac(key, buf, { returnAsBuffer: true })

    // signature.copy(buf, consts.HEADER_LENGTH, 0, 16)
    buf = put()
      .put(msg.protocolId)
      .word16le(consts.HEADER_LENGTH)
      .word16le(msg.header.creditCharge)
      ...
      .word32le(msg.header.treeId)
      .word64le(msg.header.sessionId)
      .put(signature.subarray(0, 16))
      // body
      .put(msg.body)
      .buffer()

    console.log('message2', buf.toString('hex'))

    console.log('signature', signature.subarray(0, 16).toString('hex'), signature.subarray(16, 32).toString('hex'))
  }
