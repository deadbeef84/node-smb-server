...
function handleRequest(msgBuf, connection, server, cb) {
  ...
  function processResults(err) {
    sendCompoundedResponses(compMsgs, connection, server)
...
async function sendCompoundedResponses(msgs, connection, server, cb) {
  const out = put()
  ...
  for (let n = 0; n < msgs.length; ++n) {
    ...
    let padLength = 0
    if (n < msgs.length - 1) {
      nextCommandOff = msg.body.raw ? msg.body.length : SMB2.HEADER_LENGTH + msg.body.length
    ...
    const msgbuf = await message.encode(msg, connection.signingKey, padLength)

    if (msgs.length === 1 && padLength === 0) {
      connection.sendRawMessage(msgbuf, cb)
      return
    }
...
function sendResponse(msg, status, connection, server, cb) {
  ...
  message.encode(msg, connection.signingKey).then(buf => connection.sendRawMessage(buf, cb))
}
