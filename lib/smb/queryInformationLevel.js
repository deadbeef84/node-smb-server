...
function serialize(file, informationLevel) {
  ...
  const STREAM_NAME_BYTES = Buffer.from('::$DATA', 'utf16le')
  ...
  switch (informationLevel) {
    ...
    case SMB.QUERY_FILE_COMPRESSION_INFO:
      ...
      logger.error(
        'encountered unsupported informationLevel %s',
        SMB.QUERY_INFORMATION_LEVEL_TO_STRING[informationLevel]
      )
  ...
  return { status, buffer: out.buffer() }
}
