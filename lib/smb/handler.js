...
function handleRequest(msgBuf, connection, server, cb) {
  // validate length
  if (
    msgBuf.length < SMB.SMB_MIN_LENGTH ||
    msgBuf.length > SMB.SMB_MAX_LENGTH
  ) {
    cb(
    ...
    function (cmd, callback) {
      ...
      if (!command) {
        ...
        callback({
          ...
          message:
            'encountered invalid command 0x' + cmd.commandId.toString(16),
        });
      }
      ...
      if (handler) {
        ...
        d.on('error', function (err) {
          logger.error(
            'encountered unhandled exception at the handler level. exiting',
            err
          );
          process.exit(1);
        });
