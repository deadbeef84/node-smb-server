...
const RPC = require('../../../dcerpc/constants')
const packet = require('../../../dcerpc/packet')

const pduHandlers = {}

function loadPDUHandlers() {
  const p = path.join(__dirname, '../../../dcerpc/pdu')
...
function handle(
  ...
  const fid = msg.setup.readUInt16LE(2)

  const tree = server.getTree(msg.header.tid)
  const fileName = (tree && tree.getFile(fid) && tree.getFile(fid).getName()) || null

  logger.debug(
    '[%s] fid: %d [fileName: %s], data: %s',
    SMB.TRANS_SUBCOMMAND_TO_STRING[commandId].toUpperCase(),
    fid,
    fileName,
    commandData.toString('hex')
  )
  ...
  // decode PDU
  const hdr = packet.parseCommonHeaderFields(commandData)
  ...
  const pdu = RPC.PDUTYPE_TO_STRING[hdr.type]
  if (!pdu) {
  ...
  if (handler) {
    ...
    logger.error(
      "encountered unsupported PDU type 0x%s '%s'",
      hdr.type.toString(16),
      pdu.toUpperCase()
    )
    result = {
      status: ntstatus.STATUS_NOT_IMPLEMENTED,
