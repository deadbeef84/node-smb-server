...
function handle(
  ...
  // decode params
  const searchAttributes = commandParams.readUInt16LE(0)

  // decode data
  let off = 0
  const bufferFormat1 = commandData.readUInt8(off) // 0x04
  off += 1
  ...
  off += bytes.length + 2
  const bufferFormat2 = commandData.readUInt8(off) // 0x04
  off += 1
  off += utils.calculatePadLength(commandDataOffset + off, 2) // pad to align subsequent unicode strings (utf16le) on word boundary
  ...
  const matchLongNames = msg.header.flags.pathnames.long.supported
  const inclHidden = !!(searchAttributes & SMB.ATTR_HIDDEN)
  const inclSystem = !!(searchAttributes & SMB.ATTR_SYSTEM)

  logger.debug(
    '[%s] searchAttributes: %s, oldFileName: %s, newFileName: %s',
