...
function handle(
  ...
  // decode params
  let parser = binary.parse(commandParams)
  const paramsObj = parser
    ...
    .word16le('numberOfRequestedLocks').vars
  ...
  logger.debug(
    '[%s] fid: %d [fileName: %s], typeOfLock: %s, newOpLockLevel: %d, timeout: 0x%s, numberOfRequestedUnlocks: %d, numberOfRequestedLocks: %d',
    SMB.COMMAND_TO_STRING[commandId].toUpperCase(),
    msg.fid,
    fileName,
    msg.typeOfLock.toString(2),
    msg.newOpLockLevel,
    msg.timeout.toString(16),
    msg.numberOfRequestedUnlocks,
    ...
    if (largeFiles) {
      ...
      result.push(
        parser
          ...
          .word32le('lengthInBytesLow').vars
      )
    } else {
      // 32bit offset & length
      result.push(parser.word16le('pid').word32le('byteOffset').word32le('lengthInBytes').vars)
    }
  }
  return result
}
