...
function handle(
  ...
  msg.unlocks = parseRanges(
    parser,
    msg.numberOfRequestedUnlocks,
    msg.typeOfLock & SMB.LARGE_FILES
  );
  msg.locks = parseRanges(
    parser,
  ...
  var fileName =
    (tree && tree.getFile(msg.fid) && tree.getFile(msg.fid).getName()) || null;
...
function parseRanges(parser, count, largeFiles) {
  ...
  while (count-- > 0) {
    if (largeFiles) {
      ...
      // 32bit offset & length
      result.push(
        parser.word16le('pid').word32le('byteOffset').word32le('lengthInBytes')
          .vars
      );
    }
  }
