...
function handle(
  ...
  // data to be written
  const dataLength = msg.dataLength + (msg.dataLengthHigh << 16)
  const data = msg.buf.slice(msg.dataOffset, msg.dataOffset + dataLength)

  const tree = server.getTree(msg.header.tid)
  const fileName = (tree && tree.getFile(msg.fid) && tree.getFile(msg.fid).getName()) || null

  logger.debug(
    '[%s] fid: %d [fileName: %s], offset: %d, timeout: %d, remaining: %d, dataLengthHigh: %d, dataLength: %d, dataOffset: %d',
