...
function handle(
  ...
  // decode params
  const fileAttributes = commandParams.readUInt16LE(0)
  const lastWriteTime = commandParams.readUInt32LE(2)
  ...
  logger.debug(
    '[%s] fileAttributes: %s, lastWriteTime: %s, fileName: %s',
    SMB.COMMAND_TO_STRING[commandId].toUpperCase(),
    fileAttributes.toString(2),
    new Date(lastWriteTime * 1000).toString(),
    fileName
  )
