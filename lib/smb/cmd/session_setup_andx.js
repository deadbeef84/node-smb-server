...
function handle(
  ...
  const extendedSecurity = commandParams.length === 2 * 12
  parser
    ...
    .word32le('sessionKey')
  if (!extendedSecurity) {
    parser.word16le('caseInsensitivePasswordLength').word16le('caseSensitivePasswordLength')
  } else {
    // extended security
  ...
  function buildResult(status, uid, extSec, secBlob) {
    ...
    out
      .put(Buffer.from(server.nativeOS, 'utf16le'))
      .word16le(0)
      .put(Buffer.from(server.nativeLanMan, 'utf16le'))
      .word16le(0)
    if (!extSec) {
  ...
  const sessionKey = msg.sessionKey || connection.sessionKey
  const login = server.getLogin(sessionKey)
  let securityBlob = utils.EMPTY_BUFFER
  if (!login) {
    logger.debug(
      '[%s] setupSession failed: sessionKey: %d',
  ...
  if (extendedSecurity) {
    const msgType = ntlmssp.parseMessageType(msg.securityBlob)
    if (msgType === ntlmssp.NTLMSSP_NEGOTIATE_MESSAGE) {
      // parse NTLMSSP_NEGOTIATE msg
      const negMsg = ntlmssp.parseNegotiateMessage(msg.securityBlob)
      if (!negMsg) {
        cb(buildResult(ntstatus.STATUS_LOGON_FAILURE, 0, extendedSecurity, securityBlob))
        return
      }
      // create NTLMSSP_CHALLENGE msg
      securityBlob = ntlmssp.createChallengeMessage(
        negMsg.flags,
        login.challenge,
        server.hostName,
        server.domainName
      ...
      const authMsg = ntlmssp.parseAuthenticateMessage(msg.securityBlob)
      if (!authMsg) {
        cb(buildResult(ntstatus.STATUS_LOGON_FAILURE, 0, extendedSecurity, securityBlob))
        return
      }
  ...
  logger.debug(
    '[%s] maxBufferSize: %d, maxMpxCount: %d, vcNumber: %d, sessionKey: %d, capabilities: %s, accountName: %s, primaryDomain: %s, nativeOS: %s, nativeLanMan: %s',
    SMB.COMMAND_TO_STRING[commandId].toUpperCase(),
    msg.maxBufferSize,
    msg.maxMpxCount,
    msg.vcNumber,
    msg.sessionKey,
    msg.capabilities.toString(2),
    msg.accountName,
    msg.primaryDomain,
    msg.nativeOS,
    msg.nativeLanMan
  ...
  server.setupSession(
    ...
    msg.accountName,
    msg.primaryDomain,
    msg.caseInsensitivePassword,
    msg.caseSensitivePassword,
    function (err, session) {
      if (err) {
        logger.debug(
          '[%s] setupSession failed',
          SMB.COMMAND_TO_STRING[commandId].toUpperCase(),
          err
        )
        // authentication failure
