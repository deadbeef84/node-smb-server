...
// flags
const NT_CREATE_REQUEST_OPLOCK = 0x00000002 // If set, the client requests an exclusive OpLock.
const NT_CREATE_REQUEST_OPBATCH = 0x00000004 // If set, the client requests an exclusive batch OpLock.
const NT_CREATE_OPEN_TARGET_DIR = 0x00000008 // If set, the client indicates that the parent directory of the target is to be opened.
const NT_CREATE_REQUEST_EXTENDED_RESPONSE = 0x00000010 // If set, then the client is requesting extended information in the response.

// FileStatusFlags
const NO_EAS = 0x0001 // The file or directory has no extended attributes.
const NO_SUBSTREAMS = 0x0002 // The file or directory has no data streams other than the main data stream.
const NO_REPARSETAG = 0x0004 // The file or directory is not a reparse point.
...
function handle(
  ...
  const paramsObj = parser
    ...
    .word8('securityFlags').vars
  _.assign(msg, paramsObj)

  // decode data
  // pad to align subsequent unicode strings (utf16le) on word boundary
  const off = utils.calculatePadLength(commandDataOffset, 2)
  msg.fileName = utils.extractUnicodeBytes(commandData, off).toString('utf16le')

  logger.debug(
    '[%s] flags: %s, rootDirectoryFID: %d, desiredAccess: %s, allocationSize: %d, extFileAttributes: %s, shareAccess: %s, createDisposition: 0x%s, createOptions: %s, impersonationLevel: 0x%s, securityFlags: %s, fileName: %s',
    SMB.COMMAND_TO_STRING[commandId].toUpperCase(),
    msg.flags.toString(2),
    msg.rootDirectoryFID,
    msg.desiredAccess.toString(2),
    msg.allocationSize,
    msg.extFileAttributes.toString(2),
    msg.shareAccess.toString(2),
    msg.createDisposition.toString(16),
    msg.createOptions.toString(16),
    msg.impersonationLevel.toString(16),
    msg.securityFlags.toString(2),
    msg.fileName
  )
  ...
  function getFile(callback) {
    if (msg.rootDirectoryFID) {
      const parent = tree.getFile(msg.rootDirectoryFID)
      // todo evaluate/handle rootDirectoryFID, i.e. resolve filename relative to specified parent directory
    }
    tree.openOrCreate(
      msg.fileName,
      msg.createDisposition,
      !!(msg.createOptions & common.FILE_DIRECTORY_FILE),
      callback
    )
  }

  function buildResult(file, callback) {
    const smbCreated = utils.systemToSMBTime(file.getCreatedTime())
    const smbLastModified = utils.systemToSMBTime(file.getLastModifiedTime())
    const smbLastAccessed = utils.systemToSMBTime(file.getLastAccessedTime())
    const smbLastChanged = utils.systemToSMBTime(file.getLastChangedTime())

    // params
    const out = put()
    out
      .word8(commandParams.readUInt8(0)) // andX next cmd id
      ...
      .word16le(
        file.getTree().getShare().isNamedPipe() ? SMB.FILE_TYPE_MESSAGEMODEPIPE : SMB.FILE_TYPE_DISK
      ) // ResourceType
    if (msg.flags & NT_CREATE_REQUEST_EXTENDED_RESPONSE) {
      // MS-SMB v1.0
      out
        .word16le(NO_EAS | NO_SUBSTREAMS | NO_REPARSETAG) // FileStatusFlags
