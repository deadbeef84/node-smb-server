...
function handle(
  ...
  function buildResult(file, callback) {
    ...
    out
      ...
      .word16le(
        file.getTree().getShare().isNamedPipe()
          ? SMB.FILE_TYPE_MESSAGEMODEPIPE
          : SMB.FILE_TYPE_DISK
      ); // ResourceType
    if (msg.flags & NT_CREATE_REQUEST_EXTENDED_RESPONSE) {
      ...
      out
        ...
        .word64le(0) // FileId
        .word32le(
          file.isDirectory() ? SMB.DIRECTORY_ACCESS_ALL : SMB.FILE_ACCESS_ALL
        ) // MaximalAccessRights
        .word32le(
          file.isDirectory()
            ? SMB.DIRECTORY_ACCESS_READONLY
            : SMB.FILE_ACCESS_READONLY
