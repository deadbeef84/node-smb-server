...
function handle(
  ...
  // decode data
  let off = 0
  const bufferFormat = commandData.readUInt8(off) // 0x04
  off += 1
  off += utils.calculatePadLength(commandDataOffset + off, 2) // pad to align subsequent unicode strings (utf16le) on word boundary
  const fileName = utils.extractUnicodeBytes(commandData, off).toString('utf16le')
  ...
  logger.debug(
    '[%s] searchAttributes: %s, fileName: %s',
