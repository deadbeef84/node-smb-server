...
const infoLevel = require('../../queryInformationLevel')
...
function handle(
  ...
  const serializeResult = infoLevel.serialize(file, informationLevel)
  if (serializeResult.status !== ntstatus.STATUS_SUCCESS) {
    ...
    result = {
      status: serializeResult.status,
      params:
        informationLevel >= SMB.QUERY_FILE_BASIC_INFO
