...
function handle(
  ...
  const search = session.getSearch(sid)
  if (!search) {
    result = {
  ...
  logger.debug(
    '[%s] SID: %d, searchCount: %d, informationLevel: %s, resumeKey: %d, flags: %s, fileName: %s',
  ...
  const files = search.results

  let count = files.length - resumeKey
  if (count <= 0) {
    cb({
  ...
  const serializeResult = infoLevel.serialize(
    files,
    resumeKey,
    resumeKey + count,
    informationLevel,
