...
function handle(
  ...
  const fileName = utils.extractUnicodeBytes(commandParams, off).toString('utf16le')

  logger.debug(
    '[%s] informationLevel: %s, fileName: %s',
  ...
  function processFile(file, callback) {
    ...
    switch (informationLevel) {
      ...
      default:
        logger.error(
          '[%s] encountered unsupported or unknown informationLevel %s, fileName: %s',
          SMB.TRANS2_SUBCOMMAND_TO_STRING[commandId].toUpperCase(),
          SMB.SET_INFORMATION_LEVEL_TO_STRING[informationLevel] ||
            '0x' + informationLevel.toString(16),
          fileName
  ...
  async.waterfall([getFile, processFile, closeFile], function (err, result) {
    if (err) {
      logger.error(
        '[%s] informationLevel: %s, fileName: %s failed',
        SMB.TRANS2_SUBCOMMAND_TO_STRING[commandId].toUpperCase(),
        SMB.SET_INFORMATION_LEVEL_TO_STRING[informationLevel],
        fileName,
        err
