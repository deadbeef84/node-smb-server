...
function handle(
  ...
  const fid = commandParams.readUInt16LE(0)
  const informationLevel = commandParams.readUInt16LE(2)

  const tree = server.getTree(msg.header.tid)
  const fileName = (tree && tree.getFile(fid) && tree.getFile(fid).getName()) || null

  logger.debug(
    '[%s] informationLevel: %s, fid: %d [fileName: %s]',
    SMB.TRANS2_SUBCOMMAND_TO_STRING[commandId].toUpperCase(),
    SMB.SET_INFORMATION_LEVEL_TO_STRING[informationLevel],
    fid,
    fileName || ''
  )
  ...
  const eaErrorOffset = Buffer.from([0x00, 0x00])
  let msLastAccess, msCreation, msLastWrite, msChange

  switch (informationLevel) {
    case SMB.SET_INFO_STANDARD:
      off = 0
      let smbDate = commandData.readUInt16LE(off)
      off += 2
      let smbTime = commandData.readUInt16LE(off)
      off += 2
      ...
      logger.debug(
        '[%s] informationLevel: %s, fileName: %s, creationTime: %d, lastAccessTime: %d, lastWriteTime: %d',
        SMB.TRANS2_SUBCOMMAND_TO_STRING[commandId].toUpperCase(),
        SMB.SET_INFORMATION_LEVEL_TO_STRING[informationLevel] ||
          '0x' + informationLevel.toString(16),
        fileName,
        msCreation,
        msLastAccess,
        msLastWrite
    ...
    case SMB.SET_INFO_EAS:
      const eaList = utils.parseFEAList(commandData, 0)
    ...
    case SMB.FILE_BASIC_INFORMATION:
      ...
      off += 8
      const extFileAttributes = commandData.readUInt32LE(off)
      off += 4
      ...
      logger.debug(
        '[%s] informationLevel: %s, fileName: %s, creationTime: %d, lastAccessTime: %d, lastWriteTime: %d, changeTime: %d, extFileAttributes: %s',
        SMB.TRANS2_SUBCOMMAND_TO_STRING[commandId].toUpperCase(),
        SMB.SET_INFORMATION_LEVEL_TO_STRING[informationLevel] ||
          '0x' + informationLevel.toString(16),
        fileName,
    ...
    case SMB.FILE_DISPOSITION_INFORMATION:
      const deletePending = !!commandData.readUInt8(0)
      if (deletePending) {
        logger.debug(
          '%s, fid: %d [fileName: %s], deletePending: %d',
          SMB.SET_INFORMATION_LEVEL_TO_STRING[informationLevel],
          fid,
          fileName,
          deletePending
    ...
    case SMB.FILE_END_OF_FILE_INFORMATION:
      const endOfFile = Long.fromBits(
        commandData.readUInt32LE(0),
        commandData.readUInt32LE(4),
        true
      ).toNumber()
    ...
    case SMB.FILE_ALLOCATION_INFORMATION:
      const allocationSize = Long.fromBits(
        commandData.readUInt32LE(0),
        commandData.readUInt32LE(4),
        true
      ).toNumber()
    ...
    case SMB.FILE_RENAME_INFORMATION:
      off = 0
      const replaceIfExists = commandData.readUInt8(off)
      off += 1
      off += 3 // Reserved (padding)
      const rootDirectory = commandData.readUInt32LE(off)
      off += 4
      const fileNameLength = commandData.readUInt32LE(off)
      off += 4
      off += utils.calculatePadLength(commandDataOffset + off, 2) // pad to align subsequent unicode strings (utf16le) on word boundary
      // let bytes = commandData.slice(off, fileNameLength);
      // let targetName = bytes.toString('utf16le');
      const targetName = utils.extractUnicodeBytes(commandData, off).toString('utf16le')
      const targetPath = Path.join(utils.getParentPath(file.getPath()), targetName)
      logger.debug(
        '%s, fid: %d [fileName: %s], replaceIfExists: %d, rootDirectoryHandle: %d, fileNameLength: %d, targetName: %s',
        SMB.SET_INFORMATION_LEVEL_TO_STRING[informationLevel],
        fid,
        fileName,
        replaceIfExists,
    ...
    default:
      logger.error(
        '[%s] encountered unsupported or unknown informationLevel %s, fid: %d [fileName: %s]',
        SMB.TRANS2_SUBCOMMAND_TO_STRING[commandId].toUpperCase(),
        SMB.SET_INFORMATION_LEVEL_TO_STRING[informationLevel] ||
          '0x' + informationLevel.toString(16),
        fid,
