...
function handle(
  ...
  var fileName =
    (tree && tree.getFile(fid) && tree.getFile(fid).getName()) || null;
  ...
  if (
    informationLevel !== SMB.SET_INFO_STANDARD &&
    !msg.header.flags.pathnames.long.supported
  ) {
    result = {
  ...
  switch (informationLevel) {
    ...
    case SMB.FILE_ALLOCATION_INFORMATION:
      var allocationSize = Long.fromBits(
        commandData.readUInt32LE(0),
        commandData.readUInt32LE(4),
        true
      ).toNumber();
    ...
    case SMB.FILE_RENAME_INFORMATION:
      ...
      var targetName = utils
        .extractUnicodeBytes(commandData, off)
        .toString('utf16le');
      var targetPath = Path.join(
        utils.getParentPath(file.getPath()),
        targetName
      );
      logger.debug(
      ...
      tree.rename(file, targetPath, function (err) {
        cb({
          status: err
            ? err.status || ntstatus.STATUS_UNSUCCESSFUL
