...
const DEFAULT_SERIAL_NUMBER = 0xdabbad00
...
const FILE_SYSTEM = 'NTFS'

// fake values for disk total/free size
const SECTORS_PER_UNIT = 8
const BYTES_PER_SECTOR = 512
const TOTAL_ALLOCATION_UNITS = 0x08000000 // * SECTORS_PER_UNIT * BYTES_PER_SECTOR = 512gb
...
function handle(
  ...
  const informationLevel = commandParams.readUInt16LE(0)

  logger.debug(
    '[%s] informationLevel: %s',
    SMB.TRANS2_SUBCOMMAND_TO_STRING[commandId].toUpperCase(),
    SMB.QUERY_FS_INFORMATION_LEVEL_TO_STRING[informationLevel]
  )
  ...
  const share = tree.getShare()
  const volumeLabel = share.getDescription()
  ...
  switch (informationLevel) {
    ...
    case SMB.QUERY_FS_INFO_VOLUME:
      dataOut
        .word32le(DEFAULT_SERIAL_NUMBER) // ulVolSerialNbr
    ...
    case SMB.QUERY_FS_VOLUME_INFO:
      ...
      dataOut
        .word32le(long.getLowBitsUnsigned()) // VolumeCreationTime
    ...
    case SMB.QUERY_FS_SIZE_INFO:
      dataOut
        .word64le(TOTAL_ALLOCATION_UNITS) // TotalAllocationUnits
