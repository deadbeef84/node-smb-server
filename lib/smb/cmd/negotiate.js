...
const smb2 = require('../../smb2/handler')
const smb2Message = require('../../smb2/message')
const SMB2 = require('../../smb2/constants')

const ZERO = Buffer.from([0])

// Max multiplex count.
const MAX_MPX_COUNT = 50
// Max number of virtual circuits.
const MAX_NUMBER_VCS = 1
// Max buffer size.
const MAX_BUFFER_SIZE = 33028
// Max raw size.
const MAX_RAW_SIZE = 65536
...
function handle(
  ...
  binary.parse(commandData).loop(function (end, vars) {
    ...
    const dialect = vars.dialect.toString()
  ...
  if (!!server.config.smb2Support && msg.dialects.indexOf(SMB.DIALECT_SMB_2_X) > -1) {
    ...
    const buf = Buffer.alloc(SMB2.HEADER_LENGTH)
    buf.fill(0)
    const smb2Msg = smb2Message.decode(buf)
    ...
    const securityBuffer = utils.EMPTY_BUFFER
    const out = put()
    out
  ...
  const idx = msg.dialects.indexOf(SMB.DIALECT_NT_LM_0_12)
  ...
  let capabilities =
    SMB.CAP_STATUS32 |
    SMB.CAP_LEVEL2_OPLOCKS |
    SMB.CAP_NT_SMBS |
    SMB.CAP_NT_FIND |
    SMB.CAP_LARGE_FILES |
    SMB.CAP_UNICODE |
    SMB.CAP_INFOLEVEL_PASSTHRU |
    SMB.CAP_RPC_REMOTE_APIS |
    SMB.CAP_LARGE_READX |
    SMB.CAP_LARGE_WRITEX |
    // SMB.CAP_LWIO |
    SMB.CAP_LOCK_AND_READ

  let extendedSecurity = false
  ...
  offset += 4
  // SystemTime
  const long = utils.systemToSMBTime(Date.now())
  ...
  if (extendedSecurity) {
    ...
    const securityBlob = utils.EMPTY_BUFFER
    ...
    const encDomain = Buffer.from(server.domainName, 'utf16le')
    const encServer = Buffer.from(server.hostName, 'utf16le')
