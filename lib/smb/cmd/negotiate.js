...
function handle(
  ...
  // SMB2 handshake?
  if (
    !!server.config['smb2Support'] &&
    msg.dialects.indexOf(SMB.DIALECT_SMB_2_X) > -1
  ) {
  ...
  // SecurityMode
  params.writeUInt8(
    SMB.NEGOTIATE_USER_SECURITY | SMB.NEGOTIATE_ENCRYPT_PASSWORDS,
    offset
  );
  offset += 1;
  // MaxMpxCount
  ...
  if (extendedSecurity) {
    ...
    data = Buffer.alloc(
      login.challenge.length + encDomain.length + 2 + encServer.length + 2
    );
    // Challenge
