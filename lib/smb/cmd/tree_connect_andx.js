...
// flags
const TREE_CONNECT_ANDX_DISCONNECT_TID = 0x0001 // If set and SMB_Header.TID is valid, the tree connect specified by the TID in the SMB header of the request SHOULD be disconnected when the server sends the response.
const TREE_CONNECT_ANDX_EXTENDED_SIGNATURES = 0x0004 // If set, then the client is requesting signing key protection
const TREE_CONNECT_ANDX_EXTENDED_RESPONSE = 0x0008 // If set, then the client is requesting extended information in the SMB_COM_TREE_CONNECT_ANDX response.

// optionalSupport
const SMB_SUPPORT_SEARCH_BITS = 0x1
const SMB_SHARE_IS_IN_DFS = 0x2
// nativeFileSystem
const NATIVE_FILE_SYSTEM = 'NTFS' // todo test e.g. with 'JCR'
...
function handle(
  ...
  let bytes = utils.extractUnicodeBytes(commandData, off)
  ...
  logger.debug(
    '[%s] flags: %s, password: %s, service: %s, path: %s',
    SMB.COMMAND_TO_STRING[commandId].toUpperCase(),
    msg.flags.toString(2),
    msg.password.toString('hex'),
    msg.service,
    msg.path
  )

  const shareName = msg.path.substring(msg.path.lastIndexOf('\\') + 1)
  const shareLevelPassword = utils.bufferEquals(msg.password, Buffer.from([0x00]))
    ? null
    : msg.password
  ...
  server.connectTree(session, shareName, shareLevelPassword, function (err, tree) {
    ...
    const service = tree.getShare().isNamedPipe() ? SMB.SERVICE_NAMEDPIPE : SMB.SERVICE_DISKSHARE
    ...
    if (msg.flags & TREE_CONNECT_ANDX_EXTENDED_RESPONSE) {
      // MS-SMB v1.0
      out
        .word32le(SMB.TREE_ACCESS_ALL) // MaximalShareAccessRights
    ...
    out = put()
    out
      .put(Buffer.from(service, 'ascii'))
      .word8(0) // service
      .put(Buffer.from(NATIVE_FILE_SYSTEM, 'utf16le'))
      .word16le(0) // nativeFileSystem
