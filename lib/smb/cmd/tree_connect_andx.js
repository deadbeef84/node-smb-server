...
function handle(
  ...
  var shareLevelPassword = utils.bufferEquals(msg.password, Buffer.from([0x00]))
    ? null
    : msg.password;
  ...
  server.connectTree(
    ...
    function (err, tree) {
      ...
      var service = tree.getShare().isNamedPipe()
        ? SMB.SERVICE_NAMEDPIPE
        : SMB.SERVICE_DISKSHARE;
