...
function handle(
  ...
  if (!subCommand) {
    logger.error(
      'encountered invalid subcommand 0x%s',
  ...
  var subParams = msg.buf.slice(
    msg.parameterOffset,
    msg.parameterOffset + msg.parameterCount
  );
  ...
  if (
    msg.parameterCount < msg.totalParameterCount ||
    msg.dataCount < msg.totalDataCount
  ) {
