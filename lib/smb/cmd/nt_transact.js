...
function loadSubCmdHandlers() {
  const p = path.join(__dirname, 'nttrans')
...
function handle(
  ...
  const subCommand = SMB.NTTRANS_SUBCOMMAND_TO_STRING[msg.subCommandId]
  if (!subCommand) {
  ...
  let off = utils.calculatePadLength(commandDataOffset, 4)
  const NT_Trans_Parameters = commandData.slice(off, off + msg.parameterCount)
  off += msg.parameterCount
  // calculate pad length for 4 byte alignment
  off += utils.calculatePadLength(off, 4)
  const NT_Trans_Data = commandData.slice(off, off + msg.dataCount)
  off += msg.dataCount
  ...
  logger.debug(
    '[%s][%s] totalParameterCount: %d, parameterCount: %d, totalDataCount: %d, dataCount: %d, setup: 0x%s',
        ...
        function (result) {
          ...
          if (res && res.status === ntstatus.STATUS_SUCCESS) {
            ...
            smb.sendResponse(msg, ntstatus.STATUS_SUCCESS, connection, server, function (err) {
              if (err) {
                logger.error(
                  '[%s] failed to send notification response',
                  subCommand.toUpperCase(),
                  err
                )
              }
...
function buildResponse(subResult) {
  ...
  // build response
  const subParams = subResult.params
  const subData = subResult.data
  const setup = subResult.setup || utils.EMPTY_BUFFER

  // nt_transact response have a params length of 36 (18 words) plus length of setup
  const paramsLength = 36 + setup.length
  ...
  // data
  const dataOut = put()
  dataOut.pad(pad1).put(subParams).pad(pad2).put(subData)
