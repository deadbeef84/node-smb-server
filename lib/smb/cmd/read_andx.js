...
function handle(
  ...
  const fileName = (tree && tree.getFile(msg.fid) && tree.getFile(msg.fid).getName()) || null

  logger.debug(
    '[%s] fid: %d [fileName: %s], offset: %d, maxCountOfBytesToReturn: %d, minCountOfBytesToReturn: %d, timeoutOrMaxCountHigh: %d, remaining: %d, offsetHigh: %d',
    SMB.COMMAND_TO_STRING[commandId].toUpperCase(),
    msg.fid,
    fileName,
    msg.offset,
    msg.maxCountOfBytesToReturn,
    msg.minCountOfBytesToReturn,
    msg.timeoutOrMaxCountHigh,
    msg.remaining,
    msg.offsetHigh
  )
  ...
  if (tree && !tree.getShare().isNamedPipe()) {
    const maxCountHigh = commandParams.readUInt16LE(14) // timeoutOrMaxCountHigh
    msg.maxCountOfBytesToReturn += maxCountHigh << 16
  }

  const file = tree.getFile(msg.fid)
  if (!file) {
    result = {
  ...
  const buf = Buffer.alloc(msg.maxCountOfBytesToReturn)
  file.read(buf, 0, buf.length, msg.offset, function (err, bytesRead, buffer) {
    ...
    // data
    const data = Buffer.alloc(1 + bytesRead)
