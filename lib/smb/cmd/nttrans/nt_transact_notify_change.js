...
const FILE_NOTIFY_INFORMATION_SIZE = 12
...
function handle(
  ...
  // decode setup
  const parser = binary.parse(msg.setup)
  const setup = parser.word32le('completionFilter').word16le('fid').word8le('watchTree').vars

  const tree = server.getTree(msg.header.tid)
  const fileName = (tree && tree.getFile(setup.fid) && tree.getFile(setup.fid).getName()) || ''

  logger.debug(
    '[%s] completionFilter: %s, fid: %d [fileName: %s], watchTree: %d',
    SMB.NTTRANS_SUBCOMMAND_TO_STRING[commandId].toUpperCase(),
    setup.completionFilter.toString(2),
    setup.fid,
    fileName,
    setup.watchTree
  )
  ...
  const file = tree.getFile(setup.fid)
  if (!file) {
    result = {
  ...
  tree.registerChangeListener(
    msg.header.mid,
    file,
    !!setup.watchTree,
    setup.completionFilter,
    function (action, oldName, newName) {
...
function onChange(action, name, newName, cb) {
  let paramsOut, fileNameBytes, nextEntryOffset, pad
  switch (action) {
  ...
  if (paramsOut) {
    if (action === common.FILE_ACTION_RENAMED) {
      logger.debug(
        '[NT_TRANSACT_NOTIFY_CHANGE][%s] name: %s, newName: %s',
        common.FILE_ACTION_TO_STRING[action].toUpperCase(),
        name,
        newName
      )
    } else {
      logger.debug(
        '[NT_TRANSACT_NOTIFY_CHANGE][%s] name: %s',
        common.FILE_ACTION_TO_STRING[action].toUpperCase(),
        name
      )
    }
