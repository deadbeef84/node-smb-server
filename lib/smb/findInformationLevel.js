...
const ntstatus = require('../ntstatus.js')
const utils = require('../utils')

// Size of <code>FIND_FILE_DIRECTORY_INFO</code> (without file name).
const FIND_FILE_DIRECTORY_INFO_SIZE = 64

// Size of <code>FIND_FILE_NAMES_INFO</code> (without file name).
const FIND_FILE_NAMES_INFO_SIZE = 12

// Size of <code>FIND_FILE_FULL_DIRECTORY_INFO</code> (without file name).
const FIND_FILE_FULL_DIRECTORY_INFO_SIZE = 68

// Size of <code>FIND_FILE_BOTH_DIRECTORY_INFO</code> (without file name).
const FIND_FILE_BOTH_DIRECTORY_INFO_SIZE = 94

// Size of <code>FIND_FILE_ID_FULL_DIRECTORY_INFO_SIZE</code> (without file name).
const FIND_FILE_ID_FULL_DIRECTORY_INFO_SIZE = FIND_FILE_FULL_DIRECTORY_INFO_SIZE + 8

// Size of <code>FIND_FILE_ID_BOTH_DIRECTORY_INFO_SIZE</code> (without file name).
const FIND_FILE_ID_BOTH_DIRECTORY_INFO_SIZE = FIND_FILE_BOTH_DIRECTORY_INFO_SIZE + 2 + 8

const emptyShortNameBytes = Buffer.alloc(2 * 12) // 8.3 unicode
...
function serialize(files, start, end, informationLevel, inclResumeKey) {
  ...
  function process(file, index) {
    ...
    const fileNameBytes = Buffer.from(file.getName(), 'utf16le')
    ...
    switch (informationLevel) {
      case SMB.FIND_INFO_STANDARD:
        ...
        out
          .word16le(utils.systemToLegacySMBDate(file.getCreatedTime())) // CreationDate
      ...
      case SMB.FIND_FILE_DIRECTORY_INFO:
        out.word32le(
          index === files.length - 1 ? 0 : FIND_FILE_DIRECTORY_INFO_SIZE + fileNameBytes.length
        ) // NextEntryOffset
      ...
      case SMB.FIND_FILE_FULL_DIRECTORY_INFO:
        out.word32le(
          index === files.length - 1 ? 0 : FIND_FILE_FULL_DIRECTORY_INFO_SIZE + fileNameBytes.length
        ) // NextEntryOffset
        out.word32le(index + start) // FileIndex
        lastNameOffset = out.length()
        out
      ...
      case SMB.FIND_FILE_NAMES_INFO:
        out.word32le(
          index === files.length - 1 ? 0 : FIND_FILE_NAMES_INFO_SIZE + fileNameBytes.length
        ) // NextEntryOffset
        out.word32le(index + start) // FileIndex
        lastNameOffset = out.length()
        out
      ...
      case SMB.FIND_FILE_BOTH_DIRECTORY_INFO:
        out.word32le(
          index === files.length - 1 ? 0 : FIND_FILE_BOTH_DIRECTORY_INFO_SIZE + fileNameBytes.length
        ) // NextEntryOffset
        out.word32le(index + start) // FileIndex
        lastNameOffset = out.length()
        out
      ...
      case SMB.FIND_FILE_ID_FULL_DIRECTORY_INFO:
        out.word32le(
          index === files.length - 1
            ? 0
            : FIND_FILE_ID_FULL_DIRECTORY_INFO_SIZE + fileNameBytes.length
        ) // NextEntryOffset
        out.word32le(index + start) // FileIndex
        lastNameOffset = out.length()
        out
      ...
      case SMB.FIND_FILE_ID_BOTH_DIRECTORY_INFO:
        out.word32le(
          index === files.length - 1
            ? 0
            : FIND_FILE_ID_BOTH_DIRECTORY_INFO_SIZE + fileNameBytes.length
        ) // NextEntryOffset
        out.word32le(index + start) // FileIndex
        lastNameOffset = out.length()
        out
