...
const actions = [readConfiguration, setupServer, startServer]

async.waterfall(actions, onStarted)

/*
process.on('uncaughtException', function (err) {
  logger.error({ err }, 'encountered uncaught exception at process level. exiting')
  process.exit(1)
})
*/
...
function startServer(config, server, done) {
  const port = (config && config.listen && config.listen.port) || 445
  const host = (config && config.listen && config.listen.host) || '0.0.0.0'
