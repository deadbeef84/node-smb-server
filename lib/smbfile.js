...
function SMBFile(spiFile, smbTree, createAction, fid) {
  ...
  this.createAction =
    createAction === undefined ? common.FILE_OPENED : createAction;
  this.attributes = 0;
...
SMBFile.prototype.delete = function (cb) {
  ...
  this.smbTree.notifyChangeListeners(
    common.FILE_ACTION_REMOVED,
    this.getPath()
  );
};
...
SMBFile.prototype.setLength = function (length, cb) {
  logger.debug(
    '[fid=%d] file.setLength %s length=%d',
    this.fid,
    this.getPath(),
    length
  );
