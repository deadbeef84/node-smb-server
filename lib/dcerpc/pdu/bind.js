...
function handle(hdr, buf, pipe, server, cb) {
  ...
  const syntaxSpec = hdr.ctxs[0].syntax

  // build bind_ack/bind_nak response PDU
  const iface =
    consts.SUPPORTED_INTERFACES[syntaxSpec.uuid] &&
    consts.SUPPORTED_INTERFACES[syntaxSpec.uuid][syntaxSpec.version + '.' + syntaxSpec.versionMinor]
  ...
  if (iface) {
    ...
    logger.debug(
      'bound to %s v%d.%d (%s)',
      consts.SUPPORTED_INTERFACES[syntaxSpec.uuid].name,
      syntaxSpec.version,
      syntaxSpec.versionMinor,
      syntaxSpec.uuid
    )
    // bind_ack
    out
      .word16le(4280) // max_xmit_frag
      .word16le(4280) // max_recv_frag
      .word32le(hdr.assocGroup ? hdr.assocGroup : ++assocGroupCounter) // assoc_group_id
    // secondary address
    const secAddr = '\\PIPE\\' + pipe.getName()
    out
      .word16le(secAddr.length + 1)
      .put(Buffer.from(secAddr, 'ascii'))
      .word8(0)
    // align on 32bit boundary
    const pad2 = utils.calculatePadLength(out.length(), 4)
    if (pad2) {
      out.pad(pad2)
    }
    out
      .word8(1) // p_result_t.n_results
      .pad(1) // reserved
      .pad(2) // reserved2
    ...
    // bind_nak
    out
      .word16le(consts.BIND_REJECT_REASON_NOT_SPECIFIED) // provider_reject_reason
