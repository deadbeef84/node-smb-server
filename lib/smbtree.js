 ...
 */
SMBTree.prototype.openOrCreate = function (
  name,
  createDisposition,
  openTargetDirectory,
  cb
  ...
  if (
    createDisposition === common.FILE_OPEN ||
    createDisposition === common.FILE_OVERWRITE
  ) {
    // open existing
    open(cb);
 ...
 */
SMBTree.prototype.registerChangeListener = function (
  mid,
  file,
  deep,
  completionFilter,
  ...
  if (action === common.FILE_ACTION_RENAMED) {
    ...
    if (listener || listenerNew) {
      if (listener === listenerNew) {
        ...
        listener.cb(
          common.FILE_ACTION_RENAMED_OLD_NAME,
          trimListenerPath(name, listener)
        );
        listenerNew.cb(
