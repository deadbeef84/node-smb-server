...
DAMFile.createInstance = function (filePath, tree, content, fileLength, cb) {
  ...
  async.seq(
    ...
    if (err) {
      logger.error(
        'unexpected error while retrieving content for file %s',
        filePath,
        err
      );
      cb(
...
DAMFile.prototype.patchContent = function (content, fileLength, lastModified) {
  ...
  this.content[DAM.PROPERTIES][JCR.JCR_LASTMODIFIED] = new Date(
    lastModified
  ).toISOString();
};
...
DAMFile.prototype.lastModified = function () {
  if (this.isFile() && this.content[DAM.PROPERTIES][JCR.JCR_LASTMODIFIED]) {
    return new Date(
      this.content[DAM.PROPERTIES][JCR.JCR_LASTMODIFIED]
    ).getTime();
  } else {
    return this.created();
  }
...
DAMFile.prototype.read = function (buffer, offset, length, position, cb) {
  // call base class method
  return JCRFile.prototype.read.call(
    this,
    buffer,
    offset,
    length,
    ...
    if (err) {
      ...
      cb(
        SMBError.fromSystemError(
          err,
          'unable to delete file due to unknown error ' + self.filePath
        )
      );
