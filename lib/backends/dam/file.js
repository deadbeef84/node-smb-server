...
const JCRFile = require('../jcr/file')
const DAM = require('./constants')
const JCR = require('../jcr/constants')
 ...
 */
const DAMFile = function (filePath, content, fileLength, tree) {
    ...
    if (err) {
      ...
      cb(
        new SMBError(
          ...
          'cannot get content for file because it was not found ' + filePath
        )
      )
    } else {
...
DAMFile.prototype.toObject = function () {
  const obj = JCRFile.prototype.toObject.call(this)
...
DAMFile.prototype.delete = function (cb) {
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        SMBError.fromSystemError(err, 'unable to delete file due to unknown error ' + self.filePath)
      )
      ...
      cb(
        new SMBError(
          ...
          'unable to delete file due to unexpected status code ' +
            resp.statusCode +
            ' ' +
            self.filePath
        )
