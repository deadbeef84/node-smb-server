...
const JCRShare = require('../jcr/share')
const DAMTree = require('./tree')
 ...
 */
const DAMShare = function (name, config) {
...
DAMShare.prototype.isAssetClass = function (entity) {
  const cls = (entity && entity[DAM.CLASS]) || []
    ...
    self.getContent(parent, true, function (err, parentContent) {
      if (err) {
        ...
        if (parent == path) {
          ...
          const entities = parentContent[DAM.ENTITIES]
          if (!entities) {
            ...
            for (i = 0; i < entities.length; i++) {
              if (entities[i][DAM.PROPERTIES]) {
                const currName = entities[i][DAM.PROPERTIES][DAM.NAME]
                if (currName == name) {
...
DAMShare.prototype.parseContentChildEntries = function (content, iterator) {
  const self = this
  const entities = content[DAM.ENTITIES] || []
  entities.forEach(function (entity) {
    const nm = entity[DAM.PROPERTIES] && entity[DAM.PROPERTIES][DAM.NAME]
    if (nm) {
...
DAMShare.prototype.buildContentUrl = function (path, depth) {
  return (
    ...
    DAM.ASSETS_API_PATH +
    encodeURI(
      utils.normalizeSMBFileName(
        utils.stripParentPath(this.path, this.contextPath + DAM.DAM_ROOT_PATH) + path
      )
    ) +
    '.json'
  )
...
DAMShare.prototype.fetchContent = function (path, deep, cb) {
  ...
  const url =
    this.buildContentUrl(path, deep ? 1 : 0) +
