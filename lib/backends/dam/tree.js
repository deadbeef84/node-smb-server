 ...
 */
DAMTree.prototype.createFileInstance = function (
  filePath,
  content,
  fileLength,
  cb
  ...
  if (typeof cb !== 'function') {
    logger.error(
      new Error('DAMTree.createFileInstance: called without callback')
    );
    cb = function () {};
  }
  ...
  var req = webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        new SMBError(
          ntstatus.STATUS_UNSUCCESSFUL,
          'cannot create directory due to ' +
            resp.statusCode +
            ' response code ' +
            name
        )
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        SMBError.fromSystemError(
          err,
          'cannot delete file due to unexpected error ' + name
        )
      );
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        new SMBError(
          ntstatus.STATUS_UNSUCCESSFUL,
          'cannot delete directory due to ' +
            resp.statusCode +
            ' response code ' +
            name
        )
