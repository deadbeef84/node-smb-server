 ...
 */
JCRTree.prototype.createFileInstance = function (
  filePath,
  content,
  fileLength,
  cb
  ...
  if (typeof cb !== 'function') {
    logger.error(
      new Error('JCRTree.createFileInstance: called without callback')
    );
    cb = function () {};
  }
  if (_isDotFile(filePath)) {
    cb(
      new SMBError(
        ...
        'unable to create file instance for path names beginning with a period ' +
          filePath
      )
    );
  } else {
 ...
 */
JCRTree.prototype.createFileInstancesFromContent = function (
  content,
  path,
  namePattern,
  cb
  ...
  var self = this;
  var entries = {};
  this.share.parseContentChildEntries(
    content,
    function (childName, childContent) {
            ...
            if (!_isDotFile(f.getPath())) {
              ...
              if (uniqueResults[uniqueName]) {
                ...
                if (uniqueResults[uniqueName] < f.getName()) {
                  addFile = true;
                  logger.warn(
                    '%s excluded from list because it is duplicate of %s',
                    results[uniqueResults[uniqueName]].getPath(),
                    f.getName()
                  );
...
JCRTree.prototype.isTempFileName = function (name) {
  ...
  var names =
    name.charAt(0) === Path.sep
      ? name.substr(1).split(Path.sep)
      : name.split(Path.sep);
...
JCRTree.prototype.fetchFileLength = function (path, cb) {
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        this.method + ' ' + this.href + ' [' + resp.statusCode + '] ' + body ||
          ''
      );
    } else {
    ...
    if (err) {
      ...
      cb(
        SMBError.fromSystemError(
          err,
          'unable to determin existence due to unexpected error ' + name
        )
      );
    } else {
...
JCRTree.prototype.list = function (pattern, cb) {
  ...
  function listRemoteFiles(tmpFiles, next) {
    ...
    function createEntries(content, done) {
      self.createFileInstancesFromContent(
        content,
        parentPath,
        fileName,
        function (err, files) {
    ...
    async.waterfall([getContent, createEntries], function (err, result) {
      if (err) {
        ...
        next(
          SMBError.fromSystemError(
            err,
            'unable to list pattern due to unexpected error ' + pattern
          )
        );
      } else {
        next(null, result);
...
JCRTree.prototype.createFile = function (name, cb) {
  ...
  if (this.tempFilesTree && this.isTempFileName(name)) {
    ...
    mkdirp.sync(
      Path.join(this.tempFilesTree.share.path, utils.getParentPath(name))
    );
    ...
    webutils.submitRequest(options, function (err, resp, body) {
      if (err) {
        ...
        cb(
          SMBError.fromSystemError(
            err,
            'unable to create file due to unexpected error ' + name
          )
        );
        ...
        cb(
          new SMBError(
            ...
            'unable to create file due to 409 status code ' + name
          )
        );
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        SMBError.fromSystemError(
          err,
          'unable to create directory due to unexpected error ' + name
        )
      );
    } else if (resp.statusCode === 409) {
      cb(
        new SMBError(
          ntstatus.STATUS_OBJECT_NAME_COLLISION,
          'unable to create directory due to 409 status code ' + name
        )
      );
    } else if (resp.statusCode !== 201) {
      logger.error(
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        SMBError.fromSystemError(
          err,
          'unable to delete file due to unexpected error ' + name
        )
      );
      ...
      cb(
        new SMBError(
          ntstatus.STATUS_NO_SUCH_FILE,
          'unable to delete file because it does not exist ' + name
        )
      );
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        SMBError.fromSystemError(
          err,
          'unable to delete directory due to unexpected error ' + name
        )
      );
    } else if (resp.statusCode === 404) {
      cb(
        new SMBError(
          ntstatus.STATUS_NO_SUCH_FILE,
          'unable to delete directory because it does not exist ' + name
        )
      );
    } else if (resp.statusCode !== 204) {
      logger.error(
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        SMBError.fromSystemError(
          err,
          'unable to rename due to unexpected error ' +
            oldName +
            ' > ' +
            newName
        )
