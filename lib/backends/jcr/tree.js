...
const tmp = require('temp').track() // cleanup on exit

const Tree = require('../../spi/tree')
const JCRFile = require('./file')
 ...
 */
const JCRTree = function (share, content, tempFilesTree) {
  ...
  if (_isDotFile(filePath)) {
    cb(
      new SMBError(
        ...
        'unable to create file instance for path names beginning with a period ' + filePath
      )
    )
  } else {
      ...
      self.createFileInstance(p, entries[p], function (err, f) {
        if (err) {
          ...
          if (f.getName()) {
            ...
            const uniqueName = f.getName().toLowerCase()
...
JCRTree.prototype.isTempFileName = function (name) {
  ...
  const names = name.charAt(0) === Path.sep ? name.substr(1).split(Path.sep) : name.split(Path.sep)
...
JCRTree.prototype.exists = function (name, cb) {
  ...
  const url = this.share.buildContentUrl(name)
  const options = this.share.applyRequestDefaults({
    url,
    method: 'HEAD',
...
JCRTree.prototype.list = function (pattern, cb) {
  ...
  const fileName = utils.getPathName(pattern)
  ...
  function listRemoteFiles(tmpFiles, next) {
    ...
    async.waterfall([getContent, createEntries], function (err, result) {
      if (err) {
        ...
        next(
          SMBError.fromSystemError(err, 'unable to list pattern due to unexpected error ' + pattern)
        )
      } else {
        next(null, result)
      }
      ...
      if (err) {
        ...
        cb(
          new SMBError(
            ...
            'unable to create file due to ' + resp.statusCode + ' status code ' + name
          )
        )
      } else {
        // succeeded
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        new SMBError(
          ntstatus.STATUS_UNSUCCESSFUL,
          'unable to create directory due to ' + resp.statusCode + ' status code ' + name
        )
      )
    } else {
      // succeeded
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        new SMBError(
          ntstatus.STATUS_UNSUCCESSFUL,
          'unable to delete file due to ' + resp.statusCode + ' status code ' + name
        )
      )
    } else {
      // succeeded
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        new SMBError(
          ntstatus.STATUS_UNSUCCESSFUL,
          'unable to delete directory due to ' + resp.statusCode + ' status code ' + name
        )
      )
    } else {
      // succeeded
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        SMBError.fromSystemError(
          err,
          'unable to rename due to unexpected error ' + oldName + ' > ' + newName
        )
      )
    } else if (resp.statusCode !== 201) {
      logger.error(
      ...
      cb(
        new SMBError(
          ntstatus.STATUS_UNSUCCESSFUL,
          'unable to rename due to ' + resp.statusCode + ' status code ' + oldName + ' > ' + newName
        )
      )
    } else {
      // succeeded
