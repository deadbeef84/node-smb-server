...
var JCRShare = function (name, config) {
  ...
  this.allCacheTTL =
    typeof config.allCacheTTL === 'number' ? config.allCacheTTL : 1800000; // default: 30m
  // TTL in ms for content cache entries
  this.contentCacheTTL =
    typeof config.contentCacheTTL === 'number' ? config.contentCacheTTL : 30000; // default: 30s
  // TTL in ms for cached binaries
  this.binCacheTTL =
    typeof config.binCacheTTL === 'number' ? config.binCacheTTL : 300000; // default: 5m
...
JCRShare.prototype.isDirectoryPrimaryType = function (primaryType) {
  return (
    [JCR.NT_FOLDER, JCR.SLING_FOLDER, JCR.SLING_ORDEREDFOLDER].indexOf(
      primaryType
    ) > -1
  );
};
...
JCRShare.prototype.getLocalFile = function (path, lastModified, cb) {
  ...
  function checkCache(path, callback) {
    ...
    if (result) {
      if (
        Date.now() - result.fetched <= self.binCacheTTL &&
...
JCRShare.prototype.connect = function (session, shareLevelPassword, cb) {
  ...
  function prepopulateTempDir(content, tempDir, done) {
    ...
    fs.closeSync(
      fs.openSync(
        Path.join(tempDir, '.metadata_never_index_unless_rootfs'),
        'w'
      )
    );
