...
const Share = require('../../spi/share')
const FSShare = require('../fs/share')
const JCRTree = require('./tree')
const JCR = require('./constants')
 ...
 */
const JCRShare = function (name, config) {
...
JCRShare.prototype.buildContentUrl = function (path, depth) {
  return (
    ...
    ':' +
    this.port +
    this.jsonServletPath +
    encodeURI(utils.normalizeSMBFileName(Path.join(this.path, path))) +
    '.' +
    depth +
    '.json'
  )
...
JCRShare.prototype.getContent = function (path, deep, cb) {
  ...
  const cache = deep ? this.cachedFolderListings : this.cachedFileEntries
...
JCRShare.prototype.invalidateContentCache = function (path, deep) {
  if (this.cachedFileEntries[path]) {
    ...
    const parentPath = utils.getParentPath(path)
  ...
  const pathPrefix = path + Path.sep
...
JCRShare.prototype.fetchContent = function (path, deep, cb) {
  ...
  const url = this.buildContentUrl(path, depth)
  const opts = this.applyRequestDefaults(null, url)
...
JCRShare.prototype.getLocalFile = function (path, lastModified, cb) {
  ...
  function checkCache(path, callback) {
    let result = self.cachedBinaries[path]
    if (result) {
      if (Date.now() - result.fetched <= self.binCacheTTL && lastModified <= result.lastModified) {
        ...
        fs.stat(result.localFilePath, function (err, stats) {
          if (err) {
            logger.warn(
              'detected corrupt cache entry %s: local copy %s cannot be found',
              path,
...
JCRShare.prototype.touchLocalFile = function (path, lastModified, cb) {
  const result = this.cachedBinaries[path]
  if (result) {
    fs.stat(result.localFilePath, function (err, stats) {
      if (!err) {
...
JCRShare.prototype.fetchResource = function (path, cb) {
  ...
  const stream = this.createResourceStream(path)
  const tmpFilePath = stream.path
  ...
  const url = this.buildResourceUrl(path)
  const options = this.applyRequestDefaults(null, url)
  self.emit('downloadstart', { path })
  webutils
    .submitRequest(options)
    .on('response', function (resp) {
      if (resp.statusCode !== 200) {
        logger.error(
          'failed to spool %s to %s - %s %s [%d]',
          path,
          tmpFilePath,
          this.method,
          this.href,
    ...
    .on('end', function () {
      self.emit('downloadend', { path })
    })
    .on('error', function (err) {
      self.emit('downloaderr', { path, err })
      fs.unlink(tmpFilePath, function (ignored) {
        cb(err)
      })
    })
...
JCRShare.prototype.connect = function (session, shareLevelPassword, cb) {
  ...
  function createTempDir(content, done) {
    ...
    if (!self.config.tmpPath) {
      ...
      mkdirp(self.config.tmpPath, function (err) {
        if (!err) {
          logger.debug(
            'created local tmp directory for temporary system files: %s',
            self.config.tmpPath
          )
        }
  ...
  function connectTempTree(content, tempDir, done) {
    const tmpShare = new FSShare('tmpFiles', {
      backend: 'fs',
  ...
  async.waterfall(
    [getContent, createTempDir, prepopulateTempDir, connectTempTree, connectJCRTree],
    function (err, tree) {
      if (err) {
        const msg =
