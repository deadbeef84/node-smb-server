...
JCRFile.createInstance = function (filePath, tree, content, fileLength, cb) {
  ...
  function getFileLength(content, callback) {
    if (fileLength > -1) {
      ...
      typeof content[JCR.JCR_CONTENT][JCR.JCR_DATA_LENGTH] === 'number'
    ) {
  ...
  async.seq(
    getContent,
    getFileLength
  )(function (err, metaData, length) {
    if (err) {
      cb(
        SMBError.fromSystemError(
          err,
          'unable to create file instance due to unexpected error ' + filePath
        )
      );
    } else {
...
JCRFile.prototype.ensureGotLocalCopy = function (cb) {
  ...
  var self = this;
  this.tree.share.getLocalFile(
    this.filePath,
    this.lastModified(),
    function (err, localFilePath) {
      if (err) {
        cb(err);
      } else {
...
JCRFile.prototype.syncLocalChanges = function (cb) {
  ...
  logger.debug(
    '[%s] file.syncLocalChanges %s',
    this.tree.share.config.backend,
    this.filePath
  );
  ...
  this.ensureGotLocalCopy(function (err, localFile) {
    if (err) {
      cb(
        SMBError.fromSystemError(
          err,
          'unable to get local copy due to unexpected error ' + self.filePath
        )
      );
      return;
    }
      ...
      webutils.submitRequest(options, function (err, resp, body) {
        if (err) {
          ...
          cb(
            SMBError.fromSystemError(
              err,
              'unable to submit request due to unexpected error ' +
                self.filePath
            )
          );
          ...
          cb(
            new SMBError(
              ...
              'unable to sync local changes due to unexpecte status code ' +
                resp.statusCode +
                ' ' +
                self.filePath
            )
          ...
          fs.fstat(localFile.fd, function (err, stats) {
            if (err) {
              ...
              self.dirty = false;
              // touch cache entry
              self.tree.share.touchLocalFile(
                self.filePath,
                self.lastModified(),
                function (err) {
                  cb(err);
...
JCRFile.prototype.patchContent = function (content, fileLength, lastModified) {
  ...
  this.content[JCR.JCR_CONTENT][JCR.JCR_LASTMODIFIED] = new Date(
    lastModified
  ).toISOString();
};
...
JCRFile.prototype.isDirectory = function () {
  return this.tree.share.isDirectoryPrimaryType(
    this.content[JCR.JCR_PRIMARYTYPE]
  );
};
...
JCRFile.prototype.lastModified = function () {
  if (this.isFile() && this.content[JCR.JCR_CONTENT][JCR.JCR_LASTMODIFIED]) {
    return new Date(
      this.content[JCR.JCR_CONTENT][JCR.JCR_LASTMODIFIED]
    ).getTime();
  } else {
    return this.created();
  }
...
JCRFile.prototype.read = function (buffer, offset, length, position, cb) {
  logger.debug(
    '[%s] file.read %s offset=%d, length=%d, position=%d',
    this.tree.share.config.backend,
    this.filePath,
    offset,
    length,
...
JCRFile.prototype.write = function (data, position, cb) {
  logger.debug(
    '[%s] file.write %s data.length=%d, position=%d',
    this.tree.share.config.backend,
    this.filePath,
    data.length,
    position
    ...
    fs.ftruncate(localFile.fd, length, function (err) {
      if (err) {
        cb(
          SMBError.fromSystemError(
            err,
            'unable to truncate file due to unexpected error ' + self.filePath
          )
        );
      } else {
        // update length and lastModified
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      cb(
        new SMBError(
          ntstatus.STATUS_UNSUCCESSFUL,
          'unable to delete file due to unexpected ' +
            resp.statusCode +
            ' status code ' +
            self.filePath
        )
...
JCRFile.prototype.flush = function (cb) {
  ...
  fs.fsync(this.localFile.fd, function (err) {
    if (err) {
      logger.warn(
        'failed to flush local file %s (%s)',
        self.localFile.path,
        self.filePath,
        err
      );
