 ...
 */
const JCRFile = function (filePath, content, fileLength, tree) {
...
JCRFile.prototype.syncLocalChanges = function (cb) {
  ...
  const self = this
  const url = this.tree.share.buildResourceUrl(this.filePath)
  const options = this.tree.share.applyRequestDefaults({
    url,
    method: 'PUT',
    headers: {
      ...
      webutils.submitRequest(options, function (err, resp, body) {
        if (err) {
          ...
          cb(
            SMBError.fromSystemError(
              err,
              'unable to submit request due to unexpected error ' + self.filePath
            )
          )
