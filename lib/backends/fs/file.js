...
var FSFile = function (filePath, stats, tree) {
  ...
  this.writeable = !!(
    2 & parseInt((stats.mode & parseInt('777', 8)).toString(8)[0])
  );
    ...
    if (err) {
      cb(
        SMBError.fromSystemError(
          err,
          'unable to create file due to unexpected error ' + filePath
        )
      );
    } else {
    ...
    fs.chmod(self.realPath, readOnly ? '444' : '644', function (err) {
      if (err) {
        cb(
          SMBError.fromSystemError(
            err,
            'unable to set read only status due to unepxected error ' +
              self.filePath
          )
        );
      } else {
  ...
  function openCB(err, fd) {
    if (err) {
      cb(
        SMBError.fromSystemError(
          err,
          'unable to get file descriptor due to unexpeced error ' +
            self.filePath
        )
      );
    } else {
  ...
  async.series(
    [
      ...
      function (done) {
        ...
        perflog.debug(
          '%s File.setLength.fs.truncate %d',
          self.getPath(),
          length
        );
        fs.truncate(
        ...
        if (self.isDirectory()) {
          ...
          fs.rmdir(
            ...
            SMBError.systemToSMBErrorTranslator(
              ...
              'unable to delete directory due to unexpected error ' +
                self.filePath
            )
          );
        } else {
...
FSFile.prototype.close = function (cb) {
  ...
  var callback = SMBError.systemToSMBErrorTranslator(
    cb,
