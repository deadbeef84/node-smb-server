...
class NxtFile {
  ...
  async read(req) {
    ...
    */
    ...
    const raw = Buffer.allocUnsafe(SMB2.HEADER_LENGTH + 17 + length)
    const { bytesRead } = await this.handle.read({ buffer: raw, offset: SMB2.HEADER_LENGTH + 16, length, position: offset })

    raw.writeUint16LE(17, 64) // StructureSize
    raw.writeUint8(SMB2.HEADER_LENGTH + 16, 66) // DataOffset
    raw.writeUint8(0, 67) // Reserved
    raw.writeUint32LE(bytesRead, 68)
    raw.writeUint32LE(0, 72) // DataRemaining - TODO?
    raw.writeUint32LE(0, 76) // Reserved2
    raw.raw = true
