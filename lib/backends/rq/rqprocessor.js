...
RQProcessor.prototype.abortUpload = function (itemPath) {
  ...
  if (self.activeRequests[itemPath]) {
    ...
    logger.info(
      '%s received a new request mid-upload. canceling upload.',
      itemPath
    );
...
RQProcessor.prototype.sync = function (config, cb) {
  ...
  var processItemMethod = function (item, method, processCb) {
    ...
    var handleError = function (path, method, err, immediateFail) {
      logger.error(
        'encountered exception while attempting to process local file %s',
        path,
        err
      );
      self.emit('syncerr', {
      ...
      if (immediateFail) {
        self.rq.completeRequest(item.path, item.name, function (err) {
          if (err) {
            logger.error(
              'unable to immediately remove request for path %s',
              path,
              err
            );
          }
        ...
        self.rq.incrementRetryCount(
          ...
          function (err) {
            if (err) {
              logger.error(
                'unable to update retry count for path %s',
                path,
                err
              );
            }
    ...
    if (url.match(/\/\./g)) {
      logger.warn(
        '%s: attempt to sync path containing names beginning with a period',
        path
      );
      handleError(
        path,
      ...
      var getRequest = function () {
        return webutils.submitRequest(options, function (err, resp) {
          if (err) {
            ...
            logger.debug(
              'received response with invalid status code %d',
              resp.statusCode
            );
            handleError(
              path,
            ...
            logger.debug(
              'path [%s] name [%s] request completed',
              item.path,
              item.name
            );
            self.rq.completeRequest(item.path, item.name, function (err) {
              if (err) {
                ...
                removeActiveUpload();
                self.tree.share.invalidateContentCache(
                  self.unicodeNormalize(item.path),
                  true
                );
                if (method != 'DELETE') {
                  self.tree.refreshWorkFiles(
                    self.unicodeNormalize(refPath),
                    function (err) {
                      if (err) {
                        logger.error(
                          'unable to delete work files for local file %s',
                          path,
                          err
                        );
                      }
      ...
      if (method == 'POST' || method == 'PUT') {
        ...
        fs.stat(localPath, function (err, stats) {
          if (err) {
            ...
            webutils.monitorTransferProgress(
              read,
              refPath,
              path,
              stats.size,
  ...
  self.rq.getProcessRequest(
    ...
    function (err, item) {
      if (err) {
        ...
        if (item) {
          logger.debug(
            'path [%s] name [%s] beginning to process',
            item.path,
            item.name
          );
