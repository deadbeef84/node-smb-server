...
function RQProcessor(tree, options) {
  ...
  this.rq.on('pathupdated', function (path) {
    ...
    for (const property in self.activeRequests) {
...
RQProcessor.prototype.abortUpload = function (itemPath) {
  ...
  if (self.activeRequests[itemPath]) {
    ...
    const file = self.activeRequests[itemPath].file
    self.activeRequests[itemPath].req.abort()
    self.activeRequests[itemPath] = undefined
    self.emit('syncabort', { path: itemPath, file })
    return true
  }
  return false
}
...
RQProcessor.prototype.sync = function (config, cb) {
  ...
  self.currCb = cb
  const processItemMethod = function (item, method, processCb) {
    const refPath = Path.join(item.path, item.name)
    const removeActiveUpload = function () {
    ...
    const handleError = function (path, method, err, immediateFail) {
    ...
    let remotePrefix = item.remotePrefix
    if (remotePrefix.charAt(remotePrefix.length - 1) == '/') {
      remotePrefix = remotePrefix.substr(0, remotePrefix.length - 1)
    }
    const url = remotePrefix + encodeURI(utils.normalizeSMBFileName(refPath))
    const path = Path.join(item.localPrefix, refPath)
    ...
    self.emit('syncstart', { path: refPath, file: path, method })
    ...
    if (url.match(/\/\./g)) {
      ...
      handleError(
        path,
        method,
        'files containing names beginning with a period are forbidden',
        true
      )
    } else {
      const options = self.share.applyRequestDefaults({
        url,
        method,
        headers: {},
      })
      ...
      const getRequest = function () {
        return webutils.submitRequest(options, function (err, resp) {
          if (err) {
            ...
            logger.debug(
              'path [%s] name [%s] received locked status, indicating file is checked out',
              item.path,
              item.name
            )
      ...
      if (method == 'POST' || method == 'PUT') {
        ...
        let read, stats
        const localPath = self.unicodeNormalize(path)
        fs.stat(localPath, function (err, stats) {
          if (err) {
            ...
            webutils.monitorTransferProgress(read, refPath, path, stats.size, function (progress) {
              logger.debug(
                '%s: read %d of %d bytes, upload %d percent complete, rate of %d bytes/sec',
                refPath,
                progress.read,
                stats.size,
                Math.round((progress.read / stats.size) * 100),
                progress.rate
              )
