...
RQFile.prototype.cacheFile = function (cb) {
  ...
  if (self.syncDone) {
    logger.debug(
      '%s sync already done for this file instance. returning currently open file',
      self.filePath
    );
    cb(null, self.openFile);
  } else if (self.tree.isTempFileName(self.filePath)) {
    logger.debug(
      '%s is a temporary file, do not attempt to cache it',
      self.filePath
    );
    cb(null, self.openFile);
  } else {
          ...
          if (err) {
            ...
            self.local.open(self.filePath, function (err, localFile) {
              if (err) {
                ...
                if (
                  localFile.lastModified() === self.lastModified() &&
                  self.openFile.isCreatedLocally
                ) {
                  logger.debug(
                    '%s file in cache is the same as in-memory, using in-memory',
                    self.getPath()
                  );
                ...
                self.openFile.close(function (err) {
                  if (err) {
                    ...
                    if (localFile.isCreatedLocally()) {
                      logger.debug(
                        'file %s was created locally, returning cached file',
                        self.filePath
                      );
                      cb(null, localFile);
                    } else {
                      logger.debug(
                        'file %s was cached from remote, check to see if it needs to be updated',
                        self.filePath
                      );
                      rqlog.debug(
                        'RQFile.cacheFile.remote.open %s',
                        self.filePath
                      );
                      self.remote.open(
                        self.tree.remoteEncodePath(self.filePath),
                          ...
                          if (err) {
                            ...
                            localFile.canDelete(function (err, canDelete) {
                              if (err) {
                                ...
                                logger.debug(
                                  'file %s cannot be safely deleted. check to see if it is in the queue',
                                  self.filePath
                                );
                                self.tree.queueDataExists(
                                  self.getPath(),
                                  function (err, exists) {
                                    if (err) {
                                      self.tree.handleErr(cb, err);
                                      ...
                                      if (!exists) {
                                        logger.debug(
                                          'file %s is not queued, send conflict event',
                                          self.filePath
                                        );
                                        self.tree.emitSyncConflict(
                                          self.getPath()
                                        );
                                      }
                                      cb(null, localFile);
                                ...
                                logger.debug(
                                  'file %s can be safely deleted. check to see if it needs to be re-synced',
                                  self.filePath
                                );
                                var remoteModified = remoteFile.lastModified();
                                // check if the remote version has been modified since it was cached
                                if (
                                  localFile.getDownloadedRemoteModifiedDate() !=
                                  remoteModified
                                ) {
                                  logger.info(
                                    'file %s has been modified remotely (%d) since it was last synced (%d). re-syncing',
                                    self.filePath,
                                    remoteModified,
                                    localFile.getLastSyncDate()
                                  );
                                  ...
                                  logger.debug(
                                    'file %s has not been modified remotely (%d) since it was last synced (%d). using cache.',
                                    self.filePath,
                                    remoteModified,
                                    localFile.getLastSyncDate()
                                  );
            ...
            logger.debug(
              '%d local file %s does not exist. fetching from remote',
              new Date().getTime(),
              self.filePath
            );
            rqlog.debug(
              'RQFile.cacheFile.share.fetchResource %s',
              self.filePath
            );
            self.local.download(
              self.remote,
              self.getPath(),
              function (err, cachedFile) {
                if (err) {
                  self.tree.handleErr(cb, err);
                } else {
      ...
      if (self.dirty) {
        ...
        self.tree.isCreatedLocally(self.getPath(), function (err, isLocal) {
          if (err) {
            ...
            logger.debug(
              '%s is newly created, queuing creation',
              self.getPath()
            );
