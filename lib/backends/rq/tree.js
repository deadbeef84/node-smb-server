...
var ignore_names = [/^\.metadata_never_index*/, /^\.aem$/, /^\.DS_Store/];
...
var RQTree = function (share, remote, options) {
  ...
  this.remote = remote;
  this.local = new RQLocalTree(
    share,
    new FSTree(new FSShare('rqlocal', share.config.local)),
    this
  );
  this.share = share;
  ...
  _existsLocally.call(this, name, function (err, localExists) {
    if (err) {
      ...
      if (localExists) {
        ...
        logger.error(
          '[rq] attempting to open path that does not exist %s',
          name
        );
        cb(
          new SMBError(
            ntstatus.STATUS_NO_SUCH_FILE,
            'cannot open file because it does not exist ' + name
          )
        );
      }
    }
...
function _prepListData(tree, pattern, cb) {
  ...
  tree.rq.getRequests(
    tree.remoteEncodePath(data.parentPath),
...
function _convertRemoteToRqFile(data, cb) {
  ...
  tree.remote.list(pattern, function (err, remoteFiles) {
    if (err) {
      ...
      async.each(
        remoteFiles,
        function (remoteFile, eachCb) {
          if (tree.isTempFileName(remoteFile.getPath())) {
            // don't include remote temp files in lists
            eachCb();
          } else {
            if (
              existingRequests[tree.remoteEncodePath(remoteFile.getName())] !=
              'DELETE'
            ) {
              tree.createFileInstanceFromOpen(
                remoteFile,
...
function _deleteLocalFile(tree, localFile, cb) {
  logger.debug(
    'local file %s was not created locally, determining if it is safe to delete',
    localFile.getPath()
  );
  ...
  localFile.canDelete(function (err, canDelete) {
    if (err) {
      ...
      logger.debug(
        'local file %s can be safely deleted locally. deleting',
        localFile.getPath()
      );
      ...
      if (localFile.isDirectory()) {
        tree.deleteLocalDirectoryRecursive(localFile.getPath(), function (err) {
          if (err) {
            ...
            logger.info(
              'directory %s was deleted remotely. exclude from list',
              localFile.getPath()
            );
            cb();
          }
        ...
        logger.info(
          'file %s was deleted remotely. exclude from file list',
          localFile.getPath()
        );
          ...
          function (localFile, eachCb) {
            if (_ignoreList(localFile.getName())) {
              ...
              if (remoteIndex !== undefined) {
                logger.debug(
                  'local file %s is present in both local and remote sources. using local info',
                  localFile.getPath()
                );
                tree.createFileInstanceFromOpen(
                  localFile,
                ...
                logger.debug(
                  'local file %s is only local, determining if it should be included',
                  localFile.getPath()
                );
                tree.createFileInstanceFromOpen(
                  localFile,
                  function (err, rqFile) {
                    if (err) {
                      ...
                      logger.debug(
                        'checking to see if file %s was created locally',
                        localFile.getPath()
                      );
                      tree.isCreatedLocally(
                        localFile.getPath(),
                        function (err, exists) {
                          if (err) {
                            eachCb(err);
                            ...
                            if (exists) {
                              logger.debug(
                                'local file %s was created locally, including in results',
                                localFile.getPath()
                              );
                              data.rqFiles.push(rqFile);
                              eachCb();
                              ...
                              _deleteLocalFile(
                                tree,
                                localFile,
                                function (err, isConflict) {
                                  if (err) {
                                    eachCb(err);
                                  } else {
                                    if (isConflict) {
                                      logger.info(
                                        'file %s is in conflict because it might need to be deleted. sending event',
                                        rqFile.getPath()
                                      );
                                      data.rqFiles.push(rqFile);
...
RQTree.prototype.canDelete = function (path, cb) {
  ...
  self.local.exists(path, function (err, exists) {
    if (err) {
      ...
      logger.debug(
        '%s has not been cached locally, so it can be deleted',
        path
      );
      cb(null, true);
    }
        ...
        if (err) {
          ...
          async.eachSeries(
            ...
            function (item, eachCb) {
              if (item.isDirectory()) {
                logger.debug(
                  '%s is a directory, adding to dir queue',
                  item.getPath()
                );
                ...
                _deleteFile(tree, item, function (err, wasDeleted) {
                  if (err) {
                    ...
                    if (!wasDeleted) {
                      logger.debug(
                        'cannot delete file %s, skipping directory',
                        item.getPath()
                      );
...
function _deleteFile(tree, toDelete, cb) {
  ...
  toDelete.canDelete(function (err, canDelete) {
    if (err) {
      ...
      logger.debug(
        'cannot delete file %s, emitting conflict event',
        toDelete.getPath()
      );
  ...
  self.local.createDirectory(name, function (err, file) {
    if (err) {
      ...
      if (!self.isTempFileName(name)) {
        self.remote.createDirectory(
          self.remoteEncodePath(name),
          function (err, remoteDir) {
            if (err) {
              self.handleErr(cb, err);
            } else {
              self.createFileInstanceFromOpen(file, cb);
  ...
  self.local.getPathInfo(name, function (err, exists) {
    if (err) {
      ...
      if (exists) {
                  ...
                  if (err) {
                    ...
                    if (!createExists || queueExists) {
                      self.queueData(name, 'DELETE', false, function (err) {
                        if (err) {
                          logger.error(
                            'unexpected error while trying to queue delete',
                            err
                          );
                        }
                        cb();
        ...
        logger.debug(
          '%s to delete does not exist locally, just queueing request',
          name
        );
        self.queueData(name, 'DELETE', false, function (err) {
          if (err) {
            logger.error(
              'unexpected error while trying to queue remote-only delete',
              err
            );
          }
          cb();
        ...
        logger.error(
          '[rq] attempting to delete path %s, which does not exist',
          name
        );
          ...
          if (exists) {
            self.remote.open(remotePath, function (err, remoteFile) {
              if (err) {
                ...
                logger.debug(
                  '%s target of rename exists remotely, using remote file info',
                  newName
                );
...
RQTree.prototype.rename = function (oldName, newName, cb) {
  ...
  logger.debug(
    '[%s] tree.rename %s to %s',
    this.share.config.backend,
    oldName,
    newName
  );
  var self = this;
  ...
  var renameRemoteDir = function (callback) {
    ...
    if (!self.isTempFileName(oldName) && !self.isTempFileName(newName)) {
      self.remote.rename(
        self.remoteEncodePath(oldName),
        self.remoteEncodePath(newName),
        callback
      );
    } else {
      logger.debug(
        'directory rename involves a temp directory, not sending to remote. %s -> %s',
        oldName,
        newName
      );
      callback();
      ...
      if (info.oldExistsLocal) {
        self.local.renameExt(oldName, newName, info.newRemote, function (err) {
          if (err) {
            ...
            // invalidate cache
            self.share.invalidateContentCache(
              utils.getParentPath(oldName),
              true
            );
            self.share.invalidateContentCache(
              utils.getParentPath(newName),
              true
            );

            if (info.isDir) {
              logger.debug(
                '%s is a directory, preparing to rename remotely',
                oldName
              );
              renameRemoteDir(function (err) {
                if (err) {
                  ...
                  logger.debug(
                    '%s successfully renamed to %s remotely',
                    oldName,
                    newName
                  );
                  self.rq.updatePath(
                    ...
                    function (err) {
                      if (err) {
                        ...
                        logger.debug(
                          'successfully updated queued requests for %s to %s',
                          oldName,
                          newName
                        );
                        self.handleErr(cb);
              ...
              self.queueData(
                ...
                'MOVE',
                newName,
                { replace: info.newExists },
                function (err) {
                  if (err) {
                    self.handleErr(cb, err);
                  } else {
                    logger.debug(
                      'successfully queued MOVE for %s -> %s',
                      oldName,
                      newName
                    );
                    self.handleErr(cb);
        ...
        logger.warn(
          'attempting to rename temp file %s, which does not exist',
          oldName
        );
