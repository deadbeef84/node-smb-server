...
const rqlog = require('winston').loggers.get('rq')
const unorm = require('unorm')
const async = require('async')

const File = require('../../spi/file')
const Tree = require('../../spi/tree')
const ntstatus = require('../../ntstatus')
const SMBError = require('../../smberror')
const utils = require('../../utils')
const FSShare = require('../fs/share')
const FSTree = require('../fs/tree')
const RQLocalTree = require('./localtree')
const RQFile = require('./file')
const RequestQueue = require('./requestqueue')
const RQProcessor = require('./rqprocessor')
const constants = require('./common')

const ignore_names = [/^\.metadata_never_index*/, /^\.aem$/, /^\.DS_Store/]
 ...
 */
const RQTree = function (share, remote, options) {
...
RQTree.prototype.isQueuedFor = function (name, methods, cb) {
  ...
  self.rq.getRequests(utils.getParentPath(name), function (err, lookup) {
    if (err) {
      ...
      let i
      const queued = lookup[utils.getPathName(name)]
      let isQueued = false
      if (queued) {
...
function _ignoreList(name) {
  for (let i = 0; i < ignore_names.length; i++) {
        ...
        async.each(
          ...
          function (localFile, eachCb) {
            if (_ignoreList(localFile.getName())) {
              ...
              const remoteIndex = lookup[localFile.getName()]
...
RQTree.prototype.list = function (pattern, cb) {
  ...
  if (filter == '*') {
    ...
    const dirPath = utils.getParentPath(pattern)
    self.share.getListCache(dirPath, self, function (err, list) {
      if (err) {
        ...
        async.waterfall(
          [async.apply(_prepListData, self, pattern), _convertRemoteToRqFile, _mergeLocalFiles],
          function (err, data) {
            if (err) {
              self.handleErr(cb, err)
            } else {
    ...
    const processRq = function (err, files) {
      if (err) {
        self.handleErr(cb, err)
      } else {
...
function _processDirectory(tree, toProcess, cb) {
  ...
  async.whilst(
    ...
    function (whilstCb) {
      const dirName = processDirs[index--].name
...
RQTree.prototype.queueDataExists = function (name, cb) {
  const self = this
  const encoded = self.remoteEncodePath(name)
...
RQTree.prototype.queueData = function (name, method, newName, moreOpts, cb) {
  ...
  let isTempFile = this.isTempFileName(name)
  const self = this
  const options = {
    method,
...
RQTree.prototype.deleteDirectory = function (name, cb) {
  ...
  const deleteRemote = function (callback) {
    if (!self.isTempFileName(name)) {
...
function _getReplaceInfo(oldName, newName, cb) {
  ...
  function _isDirectory(callback) {
    self.local.getPathInfo(oldName, function (err, exists, isDir) {
      if (err) {
        ...
        const remoteOldName = self.remoteEncodePath(oldName)
  ...
  function _getRemoteInfo(callback) {
    if (!self.isTempFileName(newName)) {
      const remotePath = self.remoteEncodePath(newName)
...
RQTree.prototype.rename = function (oldName, newName, cb) {
  ...
  const renameRemoteDir = function (callback) {
