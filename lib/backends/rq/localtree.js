      ...
      workFile.write(writeData, 0, function (writeErr) {
        workFile.close(function (closeErr) {
          if (writeErr) {
            ...
            if (closeErr) {
              logger.warn(
                'unexpected error while closing work file %s',
                filePath,
                closeErr
              );
            }
...
RQLocalTree.prototype.download = function (remote, name, cb) {
  ...
  remote.open(self.rqTree.remoteEncodePath(name), function (err, remoteFile) {
    if (err) {
      ...
      self.share.fetchResource(
        self.rqTree.remoteEncodePath(name),
        function (err, localPath) {
          if (err) {
            logger.error(
              'error while downloading file %s from remote',
              name,
              err
            );
            sendResult(err);
 ...
 */
RQLocalTree.prototype.createFromSource = function (
  source,
  remote,
  isCreated,
  cb
    ...
    if (err) {
      ...
      async.each(
        ...
        function (file, eachCb) {
          if (
            file.getName() != consts.WORK_DIR &&
            !self.isDownloading(file.getPath())
          ) {
...
RQLocalTree.prototype.deleteDirectory = function (name, cb) {
  ...
  self.list(Path.join(name, '*'), function (err, items) {
    if (err) {
      ...
      logger.debug(
        '%s is not empty, delegating delete to underlying source',
        name
      );
  ...
  function _deleteCacheInfo(deleteName, delCb) {
    self.cacheInfoExists(deleteName, function (err, exists) {
      if (err) {
        ...
        logger.debug(
          '%s rename involved cache info file already exists, removing',
          deleteName
        );
              ...
              _createWorkFile.call(
                ...
                function (createErr) {
                  renamed.close(function (closeErr) {
                    if (closeErr) {
                      logger.error(
                        'unable to close new file after rename %s',
                        newName,
                        closeErr
                      );
                    }
