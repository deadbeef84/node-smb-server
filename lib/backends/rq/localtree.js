...
const RQLocalFile = require('./localfile')

const RQLocalTree = function (share, localTree, rqTree) {
  ...
  this.rqTree = rqTree
  this.share = share
  this.cacheInfoOnly = !!share.config.cacheInfoOnly
...
function _getWorkingFilePath(path) {
  const parent = utils.getParentPath(path)
  const name = utils.getPathName(path)
...
function _writeWorkData(workFile, cacheInfo, cb) {
  const filePath = workFile.getPath()
  logger.debug('_writeWorkData: entering with filePath=%s', filePath)
  const writeData = Buffer.from(JSON.stringify(cacheInfo))
...
function _createWorkFile(targetFile, created, isRefreshed, remoteFile, cb) {
    ...
    self.source.createFile(path, function (err, workFile) {
      if (err) {
        ...
        _writeWorkData.call(
          self,
          workFile,
          RQLocalFile.getCacheInfo(targetFile, remoteFile, created, isRefreshed),
          function (err) {
            if (err) {
              cb(err)
            } else {
  ...
  if (!self.isTempFileName(name) && !targetFile.isDirectory()) {
    const filePath = self.getInfoFilePath(name)
    ...
    self.source.exists(filePath, function (err, exists) {
      if (err) {
        ...
        if (created) {
          logger.warn(
            '%s work file already exists and file is being created. re-creating work file',
            filePath
          )
    ...
    self.source.exists(path, function (err, exists) {
      if (err) {
        ...
        self.cacheInfoExists(path, function (err, cacheExists) {
          const checkDir = exists
          let doesExist = exists
...
RQLocalTree.prototype.isCreatedLocally = function (path, cb) {
  ...
  self.cacheInfoExists(path, function (err, exists) {
    if (err) {
      ...
      const infoFile = self.getInfoFilePath(path)
...
RQLocalTree.prototype.deleteDirectory = function (name, cb) {
  ...
  function _deleteWorkDir(delCb) {
    const workDir = Path.join(name, consts.WORK_DIR)
              ...
              _createWorkFile.call(
                ...
                function (createErr) {
                  renamed.close(function (closeErr) {
                    ...
                    if (createErr) {
                      logger.error(
                        'unable to create new work file after rename %s',
                        newName,
                        createErr
                      )
                      cb(createErr)
