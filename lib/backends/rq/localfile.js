 ...
 */
const RQLocalFile = function (source, cacheData, tree) {
...
RQLocalFile.readCacheInfo = function (infoFile, cb) {
  const buffer = Buffer.alloc(infoFile.size())
  infoFile.read(buffer, 0, infoFile.size(), 0, function (err) {
    if (err) {
      ...
      try {
        ...
        logger.warn(
          'trying to read work file whose contents are not json %s',
          infoFile.getPath(),
          e
        )
      }
...
RQLocalFile.prototype.getDownloadedRemoteModifiedDate = function () {
  const modified = this.data.remote.lastModified
  return modified || 0
}
...
RQLocalFile.prototype.canDelete = function (cb) {
  ...
  if (self.isDirectory()) {
    ...
    const lastModifiedAtCache = self.data.local.lastModified ? self.data.local.lastModified : 0

    logger.debug(
      '%s is a file, checking lastModified(%d)==lastModifiedAtCache(%d), not isCreatedLocally(%s), and has a downloadedRemoteModifiedDate(%d)',
      path,
      self.lastModified(),
      lastModifiedAtCache,
      self.isCreatedLocally(),
      self.getDownloadedRemoteModifiedDate()
    )

    const hasRemoteModified = !!self.getDownloadedRemoteModifiedDate()
    cb(
      null,
      self.source.lastModified() == lastModifiedAtCache &&
        !self.isCreatedLocally() &&
        hasRemoteModified
    )
  }
...
RQLocalFile.prototype.lastModified = function () {
  let date = this.source.lastModified()

  if (this.data.remote.lastModified && !this.isCreatedLocally()) {
    if (
      date == this.data.local.lastModified &&
