...
const log = require('winston').loggers.get('spi')
const rqlog = require('winston').loggers.get('rq')
const Datastore = require('nedb')

const utils = require('../../utils')
const consts = require('./common.js')
...
RequestQueue.prototype.getRequests = function (path, callback) {
  ...
  const self = this
  self.db.find({ path }, function (err, docs) {
    if (err) {
...
RequestQueue.prototype.exists = function (path, name, callback) {
  ...
  const self = this
  self.db.findOne({ $and: [{ path }, { name }] }, function (err, doc) {
    if (err) {
      callback(err)
    } else {
...
RequestQueue.prototype.purgeFailedRequests = function (maxRetries, callback) {
  ...
  self.db.find({ retries: { $gte: maxRetries } }, function (err, docs) {
    const paths = []
    const purgeRequest = function (index) {
...
RequestQueue.prototype.incrementRetryCount = function (path, name, delay, callback) {
  ...
  const newTime = new Date().getTime() + delay
  this.db.update(
    { $and: [{ path }, { name }] },
    {
      $inc: { retries: 1 },
...
RequestQueue.prototype.removeRequest = function (path, name, callback) {
  ...
  const self = this
  this.db.remove({ path, name }, {}, function (err, numRemoved) {
    if (err) {
      callback(err)
...
RequestQueue.prototype.getProcessRequest = function (expiration, maxRetries, callback) {
  ...
  self.db
    .find({
      $and: [{ timestamp: { $lte: expired } }, { retries: { $lt: maxRetries } }],
    })
    .sort({ timestamp: 1 })
    .limit(1)
...
RequestQueue.prototype.getFindPathFilter = function (path) {
  ...
  const subReg = new RegExp('^' + subPath, 'g')
  return { $or: [{ path }, { path: subReg }] }
}
...
RequestQueue.prototype.updatePath = function (oldPath, newPath, callback) {
  ...
  self.db.find(self.getFindPathFilter(oldPath), function (err, docs) {
    if (err) {
      ...
      const updateDoc = function (index) {
        if (index < docs.length) {
          self.db.update(
            { _id: docs[index]._id },
            {
...
RequestQueue.prototype.copyPath = function (oldPath, newPath, callback) {
  ...
  self.db.find(self.getFindPathFilter(oldPath), function (err, docs) {
    if (err) {
      ...
      const doInsert = function (insertIndex) {
        if (insertIndex < docs.length) {
          const doc = docs[insertIndex]
          self.queueRequest(
            {
              method: doc.method,
...
RequestQueue.prototype.queueRequest = function (options, callback) {
  ...
  const path = utils.getParentPath(fullPath)
  const name = utils.getPathName(fullPath)
  ...
  if (fullPath.match(/\/\./g)) {
    ...
    const remove = function (removeDoc, removeCallback) {
      log.debug(
      ...
      self.db.findOne({ $and: [{ path: processPath, name: processName }] }, function (err, doc) {
        if (err) {
          ...
          if (doc !== null) {
            ...
            const update = doc.method == 'PUT' || doc.method == 'POST'
    ...
    const handleResult = function (err, sendUpdate, resultPath, resultName) {
      if (err) {
        callback(err)
      } else {
        if (sendUpdate) {
