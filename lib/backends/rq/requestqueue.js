...
function RequestQueue(options) {
  ...
  this.db = new Datastore({
    filename: Path.join(options.path, consts.REQUEST_DB),
    autoload: true,
  });
}
...
RequestQueue.prototype.getRequests = function (path, callback) {
  ...
  self.db.find({ path: path }, function (err, docs) {
    if (err) {
      log.warn(
        'unexpected error while attempting to query request queue: ' + err
      );
      callback(err);
    } else {
      log.debug(
        'getCreateRequests: query for path %s returned %s records',
        path,
        docs.length
      );
 ...
 */
RequestQueue.prototype.incrementRetryCount = function (
  path,
  name,
  delay,
  callback
  ...
  rqlog.debug(
    'RequestQueue.incrementRetryCount [%s] [%s] [%d]',
    path,
    name,
    delay
  );
  ...
  this.db.update(
    ...
    function (err, numAffected) {
      if (err) {
        ...
        callback(
          'unexpected number of requests had retry count updated: ' +
            numAffected
        );
      } else {
        callback();
 ...
 */
RequestQueue.prototype.getProcessRequest = function (
  expiration,
  maxRetries,
  callback
) {
  rqlog.debug(
    'RequestQueue.getProcessRequest [%d] [%d]',
    expiration,
    maxRetries
  );
  var self = this;
  ...
  log.debug(
    'getProcessRequest: retrieving requests that are ready to be processed'
  );
  self.db
    .find({
      $and: [
        { timestamp: { $lte: expired } },
    ...
    .exec(function (err, docs) {
      if (err) {
        ...
        if (docs.length) {
          log.debug(
            'getProcessRequest: found a request for path [%s] name [%s] to process',
            docs[0].path,
            docs[0].name
          );
          ...
          self.queueRequest(
            {
              method: doc.method,
              path:
                self.getNewPath(doc.path, oldPath, newPath) + '/' + doc.name,
...
RequestQueue.prototype.queueRequest = function (options, callback) {
  ...
  if (fullPath.match(/\/\./g)) {
    ...
    callback(
      '%s: paths with names beginning with a period are forbidden from being queued for requests',
      fullPath
    );
  } else {
    var remove = function (removeDoc, removeCallback) {
      log.debug(
        'queueRequest: %s: removing previously queued %s request',
        fullPath,
        removeDoc.method
      );
      self.db.remove({ _id: removeDoc._id }, {}, function (err, numRemoved) {
        if (err) {
          log.warn(
            'queueRequest: %s: encountered error while attempting removal',
            fullPath,
            err
          );
          removeCallback(err);
          ...
          log.debug(
            'queueRequest: %s: successfully removed previously queued request',
            fullPath
          );
          removeCallback();
        }
    ...
    var insert = function (
      ...
      log.debug(
        'queueRequest: %s: preparing to insert ' + insertReqMethod + ' request',
        fullPath
      );
    ...
    var processMethod = function (
      ...
      self.db.findOne(
        { $and: [{ path: processPath, name: processName }] },
        function (err, doc) {
          if (err) {
            log.warn(
              'queueRequest: %s: unexpected error while retrieving existing requests',
              fullPath,
              err
            );
            processCallback(err);
            ...
            log.debug(
              'queueRequest: %s: finished querying for cached file %s',
              fullPath,
              processPath
            );
            if (doc !== null) {
              log.debug(
                'queueRequest: %s: already queued for %s',
                fullPath,
                doc.method
              );
              ...
              if (methodToProcess == 'DELETE') {
                ...
                log.debug(
                  'queueRequest: %s: queuing for delete. removing previously queued %s',
                  fullPath,
                  doc.method
                );
                ...
                // update timestamp
                log.debug(
                  'queueRequest: %s: updating timestamp of existing record',
                  fullPath
                );
                self.db.update(
                  { _id: doc._id },
                ...
                log.debug(
                  'queueRequest: %s: %s previously queued. changing to POST %s',
                  fullPath,
                  doc.method,
                  processPath
                );
                ...
                log.warn(
                  'queueRequest: %s: unhandled method: ' + doc.method,
                  fullPath
                );
                processCallback(null, update, processPath, processName);
              }
              ...
              log.debug(
                'queueRequest: %s: queuing originally submitted %s to %s',
                fullPath,
                methodToProcess,
                processPath
              );
    ...
    if (reqMethod == 'COPY') {
      log.debug(
        'queueRequest: %s: queueing for COPY. processing PUT for destination',
        fullPath,
        reqMethod
      );
      ...
      log.debug(
        'queueRequest: %s: queueing for MOVE. processing DELETE for source',
        fullPath,
        reqMethod
      );
      ...
      processMethod('DELETE', path, name, null, null, function (err) {
        if (err) {
          ...
          log.debug(
            'queueRequest: %s: queueing for MOVE. processing %w for destination',
            fullPath,
            reqMethod,
            moveTarget
          );
