...
import { NxtShare } from './backends/nxt.js'
import { IpcShare } from './backends/ipc.js'
...
class SMBServer extends EventEmitter {
  constructor(config, authenticator) {
    ...
    // add IPC$ share
    this.shares.IPC$ = new IpcShare('IPC$', {})
   ...
   * @param {SMBSession} cb.session authenticated session
   */
  async connectTree(session, shareName, shareLevelPassword) {
    const share = this.shares[shareName]
    if (!share) {
      throw new Error('share not found')
    }

    const [tree, response] = await share.connect(session, shareLevelPassword)
    if (tree) {
      tree.tid = ++this.tidCounter
      this.trees[tree.tid] = tree
      this.emit('shareConnected', shareName)
    }
