...
function parseMessageType(buf) {
  ...
  if (!utils.bufferEquals(consts.NTLMSSP_SIGNATURE, sig)) {
    logger.warn(
      'invalid NTLMSSP message signature: data: 0x%s',
      sig.toString('hex')
    );
    return -1;
  }
...
function parseNegotiateMessage(buf) {
  ...
  if (
    flags & consts.NTLMSSP_NEGOTIATE_OEM_WORKSTATION_SUPPLIED &&
    workstationOffset
  ) {
...
function createChallengeMessage(
  ...
  var targetNameLen =
    negotiateFlags & consts.NTLMSSP_REQUEST_TARGET
  ...
  var targetInfoLen =
    negotiateFlags & consts.NTLMSSP_NEGOTIATE_TARGET_INFO ? info.length() : 0;
...
function parseAuthenticateMessage(buf) {
  ...
  msg.lmResponse = buf.slice(
    lmResponseOffset,
    lmResponseOffset + lmResponseLength
  );
  msg.ntResponse = buf.slice(
    ntResponseOffset,
    ntResponseOffset + ntResponseLength
  );
  ...
  msg.workstation = buf
    .slice(workstationOffset, workstationOffset + workstationLength)
    .toString('utf16le');
  msg.sessionKey = buf.slice(sessionKeyOffset, sessionKeyOffset + sessionKeyLength).toString('hex');
